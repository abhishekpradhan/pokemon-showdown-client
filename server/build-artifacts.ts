...
export function buildArtifacts(env: Record<string, string>): Plugin {
  ...
  return {
    ...
    closeBundle() {
      ...
      const source = /^[a-f0-9]{40}$/i.test(revision)
        ? `${sourceRoot.replace(/\/$/, '')}/tree/${revision}`
        : sourceRoot;
      let modified = false;
      let applicationSourceModified = false;
      let deploymentConfigurationModified = false;
      try {
        const entries = execFileSync('git', ['status', '--porcelain', '--untracked-files=normal'], {
          encoding: 'utf8',
        })
          .split('\n')
          .filter(Boolean);
        ...
        deploymentConfigurationModified =
          env.VERCEL === '1' && entries.some(entry => entry.slice(3) === 'vercel.json');
        applicationSourceModified = entries.some(
          entry => !deploymentConfigurationModified || entry.slice(3) !== 'vercel.json',
        );
      ...
      const info = {
        ...
        source,
        modified,
        applicationSourceModified,
        deploymentConfigurationModified,
        upstream: 'https://github.com/smogon/pokemon-showdown-client',
      ...
      writeFileSync(
        resolve(outDir, 'third-party-licenses.json'),
        JSON.stringify({ sourceRevision: revision, dependencies }, null, 2),
      );
      const notices = [
        'ARENA THIRD-PARTY NOTICES',
        readFileSync(resolve('UPSTREAM_NOTICES.txt'), 'utf8'),
        'Installed runtime package license/notice files; external media attribution is in docs/attribution.md.',
      ];
      for (const [path, entry] of Object.entries(lock.packages)) {
        ...
        const files = readdirSync(path, { withFileTypes: true }).filter(
          file => file.isFile() && /^(licen[sc]e|copying|notice)([.-].*)?$/i.test(file.name),
        );
      ...
      const buildId = createHash('sha256')
        .update(assets.join('\n') + shell + template + revision)
        .digest('hex')
        .slice(0, 20);
      writeFileSync(
        shellPath,
        shell.replace('</head>', `  <meta name="arena-build" content="${buildId}" />\n  </head>`),
      );
      const core = [
        '/',
      ...
      writeFileSync(
        ...
        template.replace(
          ...
          `const BUILD = ${JSON.stringify({ revision: buildId, assets: core })};`,
        ),
      );
    },
  };
