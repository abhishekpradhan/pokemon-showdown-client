...
function requestBody(req: IncomingMessage) {
  ...
  return new ReadableStream<Uint8Array>({
    start(controller) {
      data = chunk => { controller.enqueue(new Uint8Array(chunk)); if ((controller.desiredSize ?? 0) <= 0) req.pause(); };
      end = () => { cleanup(); controller.close(); };
      error = failure => { cleanup(); controller.error(failure); };
...
export function productionBoundary(): Plugin {
    ...
    middlewares.use(async (req, res, next) => {
      ...
      for (const rule of deployment.headers) {
        ...
        for (const header of rule.headers) {
          ...
          const value = development && header.key === 'Content-Security-Policy' ?
            header.value.replace("script-src 'self'", "script-src 'self' 'unsafe-inline'") : header.value;
      ...
      try {
        ...
        if (!['GET', 'HEAD'].includes(method)) { init.body = requestBody(req); init.duplex = 'half'; }
  ...
  return {
    ...
    configureServer(server) { install(server.middlewares, true); },
    configurePreviewServer(server) { install(server.middlewares, false); },
  };
}
