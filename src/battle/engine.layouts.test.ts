...
import { battleFieldPositions, battleTargetAt, type BattleRequest, type BattleSideID } from '../compat/battle-adapter';
import { createEngineBattle, feedLine, flipBattleView, loadEngine, projectEngineBattle, projectEngineLog } from './engine';
...
const fourLog = (gameType: string) => [
  '|gen|9', `|gametype|${gameType}`, ...species.map((_, index) => `|player|p${index + 1}|Player${index + 1}|1|1200`),
  ...species.map((_, index) => `|teamsize|p${index + 1}|6`), '|start',
  ...species.map((name, index) => `|switch|p${index + 1}${index < 2 ? 'a' : 'b'}: ${name}|${name}|100/100`), '|turn|1',
];
const privateSide = (id: BattleSideID, name: string) => ({ id, name: `Player${id[1]}`, pokemon: [{
  ident: `${id}: ${name}`, details: name, condition: '155/200', active: true, moves: ['tackle'], item: 'leftovers', ability: 'static', stats: { atk: 100, def: 100, spa: 100, spd: 100, spe: 100 },
}, { ident: `${id}: Eevee`, details: 'Eevee', condition: '180/180', active: false, moves: ['protect'], item: 'choicescarf', ability: 'adaptability' }] });
const p3Request = (gameType: string): BattleRequest => ({ gameType, rqid: 1, side: privateSide('p3', 'Charmander'),
...
describe('four independent battle owners', () => {
  it.each(corpus.cases)('projects recorded $scenario $seat without engine warnings or undisclosed exact HP', recording => {
    const battle = projectEngineLog(recording.frames.flatMap(frame => frame.split('\n')), { username: recording.username })!;
    ...
    for (const [index, side] of battle.sides!.entries()) {
      ...
      else expect(side.team.every(pokemon => pokemon.currentHp === undefined && pokemon.maxHp === undefined && pokemon.stats === undefined)).toBe(true);
    }
  });
  it.each(['multi', 'freeforall'])('retains every seat through %s private requests, damage and switches', gameType => {
    ...
    let battle = projectEngineBattle(engine, { roomId: 'battle-four', perspective: 'p3', lastRequest: request });
    expect(battle.active).toMatchObject({ name: 'Charmander', slot: 1, sideId: 'p3', currentHp: 155, maxHp: 200 });
    expect(battle.sides?.map(side => side.actives.map(pokemon => pokemon.name))).toEqual(species.map(name => [name]));
    expect(battleTargetAt(battle, -1)).toMatchObject({ sideId: 'p1', relation: gameType === 'multi' ? 'ally' : 'opponent' });
    ...
    for (const line of ['|-damage|p3b: Charmander|50/200', '|move|p4b: Squirtle|Tackle|p3b: Charmander', '|turn|2']) expect(feedLine(engine, line)).toBe(true);
  ...
  it('replays four viewpoints while keeping undisclosed rosters and exact HP hidden', () => {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      expect(battle.sides!.flatMap(side => side.team).every(pokemon => pokemon.currentHp === undefined && pokemon.item === undefined)).toBe(true);
  ...
  it('advances every owner’s Toxic counter once per upkeep', () => {
    const engine = createEngineBattle(null)!;
    for (const line of [...fourLog('multi'), ...species.map((name, index) => `|-status|p${index + 1}${index < 2 ? 'a' : 'b'}: ${name}|tox`), '|upkeep']) expect(feedLine(engine, line)).toBe(true);
  ...
  it('projects triples swap events and all six active positions', () => {
    const engine = createEngineBattle(null)!;
    for (const line of ['|gen|6', '|gametype|triples', '|start', ...['Pikachu', 'Eevee', 'Charmander'].flatMap((name, index) => [
      `|switch|p1${String.fromCharCode(97 + index)}: ${name}|${name}|100/100`,
      `|switch|p2${String.fromCharCode(97 + index)}: Foe${index}|${name}|100/100`,
    ]), '|turn|1', '|swap|p1a: Pikachu|1']) expect(feedLine(engine, line)).toBe(true);
