...
const routing = vi.hoisted(() => ({
  search: {} as { format?: string; user?: string; exact?: boolean },
  navigate: vi.fn(),
}));
...
describe('ladder browsing workflows', () => {
  beforeEach(() => {
    routing.search = { format: 'gen9ou' };
    routing.navigate.mockClear();
    useArenaStore.setState({
      selectedFormat: 'gen9randombattle',
      server: { ...getDefaultServerConfig(), id: 'showdown', host: 'sim3.psim.us' },
      connection: 'connected',
      formats: [
        { id: 'gen9ou', name: '[Gen 9] OU', searchShow: true },
        { id: 'gen9uu', name: '[Gen 9] UU', searchShow: true },
      ],
    });
    vi.mocked(requestPublicLadder).mockResolvedValue([
      { rank: 1, username: 'Alice', elo: 1600, gxe: 65, glicko: 1750, deviation: 110 },
    ]);
  });
  afterEach(() => {
    vi.clearAllMocks();
  ...
  it('browses formats and players without changing the selected matchmaking format', async () => {
    ...
    expect(routing.navigate).toHaveBeenLastCalledWith(
      expect.objectContaining({ search: expect.objectContaining({ format: 'gen9uu' }) }),
    );
    expect(useArenaStore.getState().selectedFormat).toBe('gen9randombattle');
    fireEvent.click(screen.getByRole('button', { name: 'Alice' }));
    expect(routing.navigate).toHaveBeenLastCalledWith(
      expect.objectContaining({ search: { format: 'gen9ou', user: 'Alice', exact: true } }),
    );
  });

  it('shows loading during refresh and restores player form fields from browser navigation', async () => {
    const view = render(<LadderScreen />);
    await screen.findByRole('button', { name: 'Alice' });
    let finish: (rows: []) => void = () => {};
    vi.mocked(requestPublicLadder).mockImplementation(
      () =>
        new Promise(resolve => {
          finish = resolve;
        }),
    ...
    finish([]);
    await screen.findByText('No ranked players yet for this format.');
    routing.search = { format: 'gen9ou', user: 'Bob', exact: true };
    view.rerender(<LadderScreen />);
    expect(screen.getByLabelText('Ladder player search')).toHaveValue('Bob');
    expect(screen.getByRole('checkbox')).toBeChecked();
    await waitFor(() =>
      expect(requestPublicLadder).toHaveBeenLastCalledWith(
        expect.any(String),
        'gen9ou',
        'Bob',
        true,
  ...
  it('requests custom-server standings from the connected protocol', async () => {
    useArenaStore.setState({
      server: { ...getDefaultServerConfig(), id: 'custom', host: 'localhost', port: 8000, secure: false },
    });
    ...
    expect(requestServerLadder).toHaveBeenCalledTimes(1);
    expect(requestPublicLadder).not.toHaveBeenCalled();
  });
});
