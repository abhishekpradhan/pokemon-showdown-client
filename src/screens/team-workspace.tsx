...
import { ArrowDown, ArrowUp, ChevronDown, ClipboardCopy, Copy, FileDown, FilePlus2, Library, Plus, Trash2, X } from 'lucide-react';
...
export function TeamWorkspace() {
  ...
  const load = (team: StoredTeam) => {
    ...
    setDraft(loadTeamDraft(team.id) || asDraft(team));
    setSelectedSlot(null); setNotice(''); setLibraryOpen(false); setRemovedSet(undefined);
    void navigate({ to: '/teambuilder', search: { team: team.id } });
  };
  const create = () => { setDraft(newDraft(state.selectedFormat)); setSelectedSlot(null); setNotice(''); setLibraryOpen(false); setRemovedSet(undefined); void navigate({ to: '/teambuilder', search: {} }); };
  const save = () => {
  ...
  const duplicate = () => {
    ...
    setDraft(copy); setRemovedSet(undefined); setNotice('Copy includes your visible edits.'); void navigate({ to: '/teambuilder', search: {} });
  };
  ...
  const applyImport = () => {
    try {
      ...
      setSelectedSlot(null); setRemovedSet(undefined); setDialog(null);
  ...
  const removeSlot = (index: number) => {
    ...
    setSelectedSlot(current => current === null || current === index ? null : current > index ? current - 1 : current);
  };
  const undoRemove = () => {
    ...
    patch({ sets }); setSelectedSlot(index); setRemovedSet(undefined);
  };

  return <section className="utility-workspace team-workspace" aria-label="Team builder" data-library-open={libraryOpen}>
    <div className="team-mobile-navigation"><h1>Teams</h1><button type="button" className="team-mobile-library-toggle" aria-expanded={libraryOpen} aria-controls="team-library-panel" onClick={() => setLibraryOpen(open => !open)}><Library size={17} /><span>Team library <small>{state.teams.length} saved</small></span><ChevronDown size={16} /></button><button type="button" className="pane-icon-button" onClick={create} aria-label="New team"><FilePlus2 size={18} /></button></div>
    ...
    <div className="workspace-stage team-canvas">
      ...
      <div className="team-roster-heading"><h2>Pokémon <span>{draft.sets.length} / 6</span></h2>{selectedSet && <button type="button" className="team-overview-button" onClick={() => { const index = selectedSlot; setSelectedSlot(null); requestAnimationFrame(() => { if (index !== null) rosterRef.current?.querySelectorAll<HTMLButtonElement>('.team-slot-body')[index]?.focus({ preventScroll: true }); }); }}>Team overview</button>}</div>
      {removedSet?.draftKey === draft.key && <div className="team-undo-remove" role="status"><span>{removedSet.set.species || 'Pokémon'} removed.</span><button type="button" onClick={undoRemove}>Undo</button></div>}
      <div ref={rosterRef} className={`team-slot-grid ${selectedSet ? 'is-editing' : ''}`} aria-label="Team slots">{Array.from({ length: draft.sets.length < 6 ? draft.sets.length + 1 : draft.sets.length }, (_, index) => {
        const set = draft.sets[index];
        if (!set) return <button type="button" className="team-slot is-empty" key={`empty-${index}`} onClick={() => { patch({ sets: [...draft.sets, { species: '', moves: [] }] }); setSelectedSlot(index); }}><Plus size={17} /><span>Add Pokémon</span></button>;
        return <div className={`team-slot ${selectedSlot === index ? 'is-selected' : ''}`} key={index}><button type="button" className="team-slot-body" aria-pressed={selectedSlot === index} aria-label={`Edit ${set.species || `slot ${index + 1}`}`} onClick={() => setSelectedSlot(index)}><span className="team-slot-heading"><TeamSprite set={set} format={draft.format} /><span><strong>{set.name || set.species || 'Choose species'}</strong><small>{getItem(set.item || '', genFromFormat(draft.format))?.name || set.item || 'No item'} · {getAbility(set.ability || '', genFromFormat(draft.format))?.name || set.ability || 'No ability'}</small></span></span><span className="team-slot-moves">{set.moves.filter(Boolean).map((move, moveIndex) => <i key={moveIndex}>{getMove(move, genFromFormat(draft.format))?.name || move}</i>)}</span></button><button type="button" className="team-slot-remove" aria-label={`Remove ${set.species || `slot ${index + 1}`}`} onClick={() => removeSlot(index)}><X size={13} /></button><div className="team-slot-order"><button type="button" aria-label={`Move slot ${index + 1} left`} disabled={!index} onClick={() => { const sets = [...draft.sets]; [sets[index - 1], sets[index]] = [sets[index], sets[index - 1]]; patch({ sets }); setSelectedSlot(index - 1); }}>←</button><button type="button" aria-label={`Move slot ${index + 1} right`} disabled={index === draft.sets.length - 1} onClick={() => { const sets = [...draft.sets]; [sets[index + 1], sets[index]] = [sets[index], sets[index + 1]]; patch({ sets }); setSelectedSlot(index + 1); }}>→</button></div></div>;
      })}</div>
      ...
      {selectedSet && selectedSlot !== null ? <section className="team-editor-panel" aria-label="Set editor"><SetEditor key={`${draft.key}:${selectedSlot}`} focusSpecies set={selectedSet} formatId={draft.format} onChange={set => patch({ sets: draft.sets.map((existing, position) => position === selectedSlot ? set : existing) })} /></section> : <p className="team-editor-hint">Choose a Pokémon to edit its moves and stats.</p>}
    </div>
