...
function RoomConversation({ requestedRoomId }: { requestedRoomId: string }) {
  ...
  useEffect(() => {
    ...
    if (!roomId && (requestedRoomId.startsWith('pm-') || connection === 'connected') && !joinError) joinRoom(requestedRoomId);
  ...
  if (joinError) return <section className="empty-state" role="alert"><h1>Unable to open this room</h1><p>{joinError}</p>
    <div className="button-row"><button type="button" className="secondary-action" onClick={() => joinRoom(requestedRoomId)}>Retry</button><Link to="/rooms">Browse rooms</Link></div></section>;
  ...
  const visibleMessages = historyQuery ? room.chat.filter(entry => `${entry.user} ${entry.message}`.toLowerCase().includes(historyQuery)) : room.chat;
  const visibleUsers = room.users.filter(user => user.toLowerCase().includes(userFilter.trim().toLowerCase()));
  ...
  return (
    <section className="room-surface" aria-label={`Room ${room.title}`}>
      <header className="room-surface-heading">
        <span className="room-surface-title">
          ...
          <span>
            <h1>{room.title}</h1>
            <small>
              {room.type === 'pm' ? 'Private messages' : 'Chat room'}
            </small>
          </span>
        </span>
        <div className="room-surface-actions">
          ...
          {room.type !== 'pm' && room.users.length > 0 && (
            <button ref={usersButtonRef} type="button" className="secondary-action" aria-expanded={roster} aria-controls={`room-users-${room.id}`} onClick={() => setRoster(value => !value)}><Users size={14} aria-hidden /> {room.users.length.toLocaleString()} users</button>
          )}
          <button
            type="button"
            className="secondary-action"
          ...
          >
            {room.type === 'pm' ? <X size={14} aria-hidden /> : <LogOut size={14} aria-hidden />}<span className="room-close-label">{room.type === 'pm' ? 'Close' : 'Leave'}</span>
          </button>
        </div>
      </header>

      <div className="room-history-tools conversation-tools">
        <label className="conversation-search"><Search size={15} aria-hidden />
          <input ref={historySearchRef} type="search" aria-label="Search chat history" placeholder="Search messages" value={historyFilter} onChange={event => { setHistoryFilter(event.currentTarget.value); setAtBottom(!event.currentTarget.value.trim()); }} />
          {historyFilter && <button type="button" className="conversation-icon" aria-label="Clear chat search" onClick={() => { setHistoryFilter(''); setAtBottom(true); historySearchRef.current?.focus(); }}><X size={14} aria-hidden /></button>}
        </label>
        <small role="status" aria-label="Chat search results">{historyQuery ? `${visibleMessages.length} ${visibleMessages.length === 1 ? 'result' : 'results'}` : 'This session'}</small>
        <button type="button" className="secondary-action conversation-export" aria-label="Export chat" title="Export up to 2,000 messages from this session" disabled={!room.chat.length} onClick={() => {
        ...
        }}><Download size={15} aria-hidden /><span>Export chat</span></button>
      </div>
      {roster && <section id={`room-users-${room.id}`} className="conversation-roster" aria-label="Room users">
        <div className="conversation-roster-tools"><label className="conversation-search"><Search size={15} aria-hidden /><input type="search" aria-label="Search room users" placeholder="Find a user" value={userFilter} onChange={event => setUserFilter(event.currentTarget.value)} /></label><button type="button" className="conversation-icon" aria-label="Close user list" onClick={() => { setRoster(false); usersButtonRef.current?.focus(); }}><X size={16} aria-hidden /></button></div>
        <div className="room-roster">{visibleUsers.map(user => <button type="button" className="secondary-action" key={user} onClick={event => { const rect = event.currentTarget.getBoundingClientRect(); setUserCard({ name: user, x: rect.left, y: rect.bottom, trigger: event.currentTarget }); }}>{user}</button>)}{!visibleUsers.length && <p className="chat-empty">No users match your search.</p>}</div>
      </section>}
      ...
      <div className="room-surface-feed" ref={feedRef} onScroll={event => {
        ...
        if (bottom || atBottom) setLastSeen(room.chat.at(-1)); setAtBottom(bottom);
      }}>
        <ChatFeed
          key={room.id}
      ...
      {!atBottom && !historyQuery && newMessages && <button type="button" className="secondary-action new-message-jump" onClick={() => { setAtBottom(true); setLastSeen(room.chat.at(-1)); }}>New messages · Jump to latest</button>}
