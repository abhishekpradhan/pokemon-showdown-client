...
const team: StoredTeam = {
  ...
  name: 'Saved team',
  format: 'gen9ou',
  sets: [{ species: 'Pikachu', gender: 'F', happiness: 0, moves: ['Thunderbolt'] }],
  packed: '',
  updatedAt: 1,
};
...
describe('team workspace draft lifecycle', () => {
  beforeEach(() => {
    ...
    useArenaStore.setState({
      teams: [{ ...team, packed: packTeam(team.sets) }],
      activeTeamId: team.id,
      selectedFormat: 'gen9ou',
      lastError: undefined,
      teamNotice: undefined,
      teamValidation: undefined,
      formats: [{ id: 'gen9ou', name: '[Gen 9] OU' }],
    });
  });
  ...
  it('saves incomplete new teams and keeps a failed save open', () => {
    ...
    const spy = vi.spyOn(methods, 'setItem').mockImplementation((key, value) => {
      if (key === TEAM_STORAGE_KEY) throw new Error('Quota');
      original(key, value);
    });
    try {
  ...
  it('recovers an unsaved draft after navigation and component remount', async () => {
    ...
    await waitFor(() =>
      expect(localStorage.getItem('ps-arena-team-draft-saved')).toContain('My unfinished changes'),
    );
    view.unmount();
    render(<TeamWorkspace />);
