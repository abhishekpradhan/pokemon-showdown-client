...
export function ReplaysScreen() {
  ...
  useEffect(() => {
    ...
    if (!focus || focus.interaction !== interactionVersion.current || (document.activeElement !== focus.opener && document.activeElement !== document.body)) return;
  ...
  const editSearch = (value: string, field: 'player' | 'format') => {
    activeSearch.current?.abort(); activeSearch.current = null;
    if (field === 'player') setSearchUser(value); else setSearchFormat(value);
    setSearchResults([]); setSearchPage(1); setSearchStatus('');
  };
  ...
  return <section className={`utility-workspace replay-workspace ${replay ? 'has-replay' : 'is-empty'}`} aria-label="Replays"
    onInputCapture={() => { interactionVersion.current++; }}
    onPointerDownCapture={() => { interactionVersion.current++; }}
    onKeyDownCapture={() => { interactionVersion.current++; }}
  >
    <div className="workspace-stage replay-stage" ref={stageRef} tabIndex={-1} aria-label="Replay player"><header className="stage-heading replay-heading"><span><small>Battle review</small><h1>Replays</h1></span><span className="replay-status" role="status">{loading ? 'Loading replay…' : status}</span></header>
      ...
      <div className="replay-canvas">{battle && <BattleField battle={battle} />}{!battle && <div className="replay-empty"><Play size={24} aria-hidden /><strong>{loading ? 'Opening your replay…' : 'Review a battle, turn by turn.'}</strong><p>Paste a replay link, import a log, or find a public replay below.</p></div>}</div>
      {replay && <>
    ...
    <aside className="workspace-inspector replay-inspector"><div className="replay-loader"><header className="inspector-heading"><span><small>Source</small><strong>Open a replay</strong></span></header><textarea aria-label="Replay log input" placeholder="Replay URL, ID or protocol log" rows={4} value={input} onChange={event => { setInput(event.currentTarget.value); setError(''); }} /><div className="button-row"><button type="button" className="primary-action" disabled={loading || !input.trim()} onClick={() => void openReplay(input)}>{loading ? 'Loading…' : 'Load replay'}</button>{loading && <button type="button" className="secondary-action" onClick={() => activeLoad.current?.abort(new Error('Replay loading cancelled.'))}>Cancel</button>}</div><label>Open log file<input type="file" accept=".log,.txt" onChange={event => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ''; if (file && file.size > MAX_REPLAY_BYTES) { setError('Replay is larger than 5 MB.'); return; } if (file) void file.text().then(text => { setInput(text); void openReplay(text); }).catch(() => setError('Could not read that file.')); }} /></label><small>Up to 5 MB, 50,000 lines and 2,000 turns.</small></div>
      <form className="replay-search" onSubmit={event => { event.preventDefault(); void search(); }}><strong>Find public replays</strong><label>Player<input aria-label="Replay search player" value={searchUser} onChange={event => editSearch(event.currentTarget.value, 'player')} /></label><label>Format<input aria-label="Replay search format" placeholder="gen9ou" value={searchFormat} onChange={event => editSearch(event.currentTarget.value, 'format')} /></label><button type="submit" className="secondary-action" disabled={searchStatus === 'Searching…'}>{searchStatus === 'Searching…' ? 'Searching…' : 'Search replays'}</button><p role="status">{searchStatus}</p></form><ol className="replay-result-list">{searchResults.slice(0, 50).map(result => <li key={result.id}><button type="button" onClick={() => openSearchResult(result)}>{result.players.join(' vs. ')}<small>{result.format}</small></button></li>)}</ol>{searchResults.length > 0 && <div className="button-row"><button type="button" disabled={searchPage <= 1} onClick={() => void search(searchPage - 1)}>Previous page</button><button type="button" disabled={searchResults.length <= 50} onClick={() => void search(searchPage + 1)}>Next page</button></div>}
