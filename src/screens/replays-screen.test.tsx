...
const routing = vi.hoisted(() => ({ search: {} as { replay?: string; turn?: number; step?: number; side?: 'p1' | 'p2' }, navigate: vi.fn() }));
vi.mock('@tanstack/react-router', () => ({ useSearch: () => routing.search, useNavigate: () => routing.navigate }));
vi.mock('../replays/replay-data', async importOriginal => ({ ...await importOriginal<typeof import('../replays/replay-data')>(), fetchReplay: vi.fn() }));
vi.mock('../components/battle-field', () => ({ BattleField: ({ battle }: { battle: ArenaBattle }) => <div data-testid="battle">{battle.playerSide === 'p2' ? battle.p2.name : battle.p1.name} / {battle.playerSide === 'p2' ? battle.p1.name : battle.p2.name} / {battle.turn}</div> }));

const address = 'https://replay.pokemonshowdown.com/gen9ou-123-secretpw';
describe('replay review workflows', () => {
  beforeEach(() => {
    localStorage.clear(); routing.navigate.mockClear(); routing.search = { replay: address, turn: 2, side: 'p1' };
    vi.mocked(fetchReplay).mockResolvedValue(parseReplayLog(log, { id: 'gen9ou-123-secretpw', url: address, private: 2 }));
  });
  ...
  it('opens a protected deep link, seeks turns, flips sides and restores URL navigation without refetching', async () => {
    ...
    expect(routing.navigate).toHaveBeenLastCalledWith(expect.objectContaining({ search: expect.objectContaining({ replay: address, turn: 3, side: 'p2' }) }));
    routing.search = { replay: address, turn: 1, side: 'p1' }; view.rerender(<ReplaysScreen />);
  ...
  it('persists bookmarks and provides narration beyond the battlefield', async () => {
    ...
    expect(JSON.parse(localStorage.getItem('ps-arena-replays-v1') || '[]')[0]).toMatchObject({ url: address, bookmarked: true, private: 2 });
    expect(screen.getByText('Battle narration').closest('details')?.querySelectorAll('li').length).toBeGreaterThan(0);
  });

  it('cancels an in-flight source and ignores a later stale response', async () => {
    vi.mocked(fetchReplay).mockImplementation((_input, signal) => new Promise((_resolve, reject) => signal.addEventListener('abort', () => reject(signal.reason))));
    render(<ReplaysScreen />);
