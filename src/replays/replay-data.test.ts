...
describe('replay loading boundaries', () => {
  it('normalizes extensions separately from private identifiers, queries and fragments', () => {
    expect(
      normalizeReplayUrl('https://replay.pokemonshowdown.com/gen9ou-123-secretpw.log?token=abc&p2#turn3'),
    ).toMatchObject({
      id: 'gen9ou-123-secretpw',
      dataUrl: 'https://replay.pokemonshowdown.com/gen9ou-123-secretpw.json?token=abc&p2',
      side: 'p2',
    });
    expect(normalizeReplayUrl('gen9ou-123').dataUrl).toBe(
  ...
  it('extracts metadata and rejects invalid or overlong logs', () => {
    ...
    expect(() => parseReplayLog(log + '\n'.concat('|message|x\n'.repeat(MAX_REPLAY_LINES)))).toThrow(
      '50,000-line',
    );
  });
  it('preserves returned privacy metadata and the original protected URL', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify({ log, private: 2, players: ['Alice', 'Bob'], format: '[Gen 9] OU' })),
        ),
    );
    const replay = await fetchReplay(
      'https://replay.pokemonshowdown.com/gen9ou-123-secretpw',
      controller.signal,
    );
    expect(replay.private).toBe(2);
    expect(replay.url).toContain('secretpw');
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining('secretpw.json'), {
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue(
        new Response(
          ']' +
            JSON.stringify([
              { id: 'gen9ou-1', players: ['Alice', 'Bob'], format: { bad: true } },
              { id: '../unsafe', players: ['Alice'] },
            ]),
        ),
      ),
    );
  ...
  it('supports legacy log-only hosts and skips malformed saved metadata', async () => {
    ...
    const loaded = await fetchReplay(
      'https://old.example/gen9ou-1.log?token=secret',
    ...
    localStorage.setItem(
      'ps-arena-replays-v1',
      JSON.stringify([
        { id: 'x', url: 'javascript:alert(1)', players: [] },
        { id: 'x', url: 'https://old.example/x', players: [null] },
      ]),
    );
