...
function HealthBar({ pokemon, hidden }: { pokemon: PokemonSet; hidden: boolean }) {
  ...
  const exact =
    pokemon.currentHp !== undefined && pokemon.maxHp !== undefined
  ...
  return (
    <div className="hp-block">
      ...
      {!hidden && <span className="hp-readout">{exact || `${Math.round(percent)}%`}</span>}
    </div>
  );
}
    ...
    <div className={`side-conditions is-${side}`} role="group" aria-label={label}>
      {conditions.map(condition => (
        <span key={condition.name}>
          {condition.name}
          {condition.layers > 1 && <i>×{condition.layers}</i>}
          {condition.duration && (
            <small>
              {' '}
              {condition.duration[0] === condition.duration[1]
                ? condition.duration[0]
                : condition.duration.join('–')}{' '}
              turns
            </small>
          )}
        </span>
...
function Combatant({
  ...
  const eventHere =
    event &&
    (event.sideId ? event.sideId === pokemon.sideId : event.side === side) &&
    (event.slot ?? 0) === position;
      ...
      <div className="combatant-nameplate">
        <div className="nameplate-row">
          <TooltipTrigger
            ...
            content={() => (
              <PokemonTooltip
                pokemon={pokemon}
                format={`gen${battle.generation || genFromFormat(battle.format)}`}
              />
            )}
          >
            <strong tabIndex={0} className="nameplate-name">
              {pokemon.name}
            </strong>
          </TooltipTrigger>
          {pokemon.level && pokemon.level !== 100 && (
            <span className="nameplate-level">L{pokemon.level}</span>
          )}
          {pokemon.gender && (
            <span className={`nameplate-gender is-${pokemon.gender}`}>
              {pokemon.gender === 'M' ? '♂' : '♀'}
            </span>
          )}
          {pokemon.terastallized && (
            <span
              ...
              style={typeStyle(pokemon.terastallized)}
              title={`Terastallized: ${pokemon.terastallized}`}
            >
              {pokemon.terastallized}
            </span>
          )}
        ...
        <div className="nameplate-tags">
          ...
          {pokemon.volatiles?.map(volatile => (
            <span className="volatile-chip" key={volatile}>
              {volatile}
            </span>
          ))}
          ...
          {!hideHealth &&
            ...
            pokemon.types?.map(type => (
              <span className="type-chip" key={type} style={typeStyle(type)}>
                {type}
              </span>
            ))}
        </div>
...
function RosterPips({
  ...
  return (
    <span className="roster-pips" role="img" aria-label={`${label}: ${remaining} of ${total} remaining`}>
      {team.map(pokemon => (
        <i
          ...
          title={
            hidden
              ? 'Unrevealed'
              : `${pokemon.name} · ${pokemon.hpKnown === false ? 'HP unknown' : `${Math.round(pokemon.hp)}%`}`
          }
        />
      ))}
      {Array.from({ length: Math.max(0, total - team.length) }, (_, index) => (
        <i key={`unknown-${index}`} data-unrevealed title="Unrevealed Pokémon" />
      ))}
    </span>
  );
}
...
function LayoutSide({
  ...
  const ally =
    battle.gameType === 'multi' && battleSideIndex(owner.id) % 2 === battleSideIndex(viewpoint) % 2 && !own;
  ...
  return (
    <section className="layout-side" aria-label={`${owner.name}'s side`} data-owner={owner.id}>
      <header className="layout-owner">
        <span>
          <strong>{owner.name}</strong>
          <small>
            {relation}
            {owner.rating > 0 ? ` · ${owner.rating}` : ''}
          </small>
        </span>
        <RosterPips
          team={owner.team}
          total={owner.teamSize}
          hidden={hardcore}
          label={`${owner.name}'s team`}
        />
      </header>
      {preview ? (
        <div className="layout-preview" aria-label={`${owner.name}'s team preview`}>
          {owner.team.map(pokemon => (
            <TooltipTrigger
              key={pokemon.slot}
              label={pokemon.name}
              content={() => <PokemonTooltip pokemon={pokemon} format={`gen${battle.generation || 9}`} />}
            >
              <span tabIndex={0} className="layout-preview-pokemon">
                <img
                  src={pokemonSprite(pokemon.species, { side: 'far', still: true }).url}
                  alt=""
                  width={40}
                  height={40}
                />
          ...
          {owner.team.length < owner.teamSize && (
            <small>{owner.teamSize - owner.team.length} Pokémon unrevealed</small>
          )}
        </div>
      ) : (
        <div className="layout-active-grid" data-count={count}>
          {positions.map(index => {
            ...
            return pokemon ? (
              ...
              <div className="layout-empty-position" key={index}>
      ...
      {!!owner.conditions.length && (
        <div className="layout-side-conditions" aria-label={`${owner.name}'s side conditions`}>
          {owner.conditions.map(condition => (
            <span key={condition.name}>
              {condition.name}
              {condition.layers > 1 ? ` ×${condition.layers}` : ''}
              {condition.duration ? ` · ${condition.duration.join('–')} turns` : ''}
            </span>
          ))}
        </div>
      )}
...
function ExpandedBattleField({
  ...
  const preview =
    battle.requestType === 'team' || (battle.turn === 0 && sides.every(side => !side.actives.length));
  const effects = [battle.weather, ...(battle.fieldConditions || [])].filter(Boolean);
  return (
    <div
      className="battle-field is-expanded"
      aria-label="Battle field"
      data-layout={battle.gameType}
      data-weather={battle.weather || 'clear'}
    >
      <div className="field-backdrop" aria-hidden />
      <div className="field-vignette" aria-hidden />
      <header className="layout-field-heading">
        <strong>
        ...
        <span>
          {preview
            ? sides.every(side => !side.team.length)
              ? 'Waiting for players'
              : 'Team preview'
            : `Turn ${battle.turn || '—'}`}
        </span>
      ...
      {(['far', 'near'] as const).map(half => {
        ...
        return (
          <div className={`layout-half is-${half}`} key={half}>
            {owners.map(owner => (
              <LayoutSide
                key={owner.id}
                owner={owner}
      ...
      {effects.length > 0 && (
        <div className="layout-field-effects" role="group" aria-label="Field conditions">
          {effects.map(effect => (
...
export function BattleField({
  ...
  const farCount = battle.opponentActives?.length
    ? Math.max(...battle.opponentActives.map(pokemon => pokemon.slot), battle.opponentActives.length)
    : 1;
  const nearCount = battle.actives?.length
    ? Math.max(...battle.actives.map(pokemon => pokemon.slot), battle.actives.length)
    : 1;
  ...
  return (
    <div className="battle-field" aria-label="Battle field" data-weather={battle.weather || 'clear'}>
      ...
      <header className="field-hud">
        <span className="field-player">
          ...
          <RosterPips
            ...
            total={battle.opponentTeamSize}
            hidden={hardcore}
            label={`${farPlayer.name}'s team`}
          />
        </span>
      ...
      <SideConditions
        conditions={battle.opponentSideConditions}
        label="Opponent's side conditions"
        side="far"
      />
