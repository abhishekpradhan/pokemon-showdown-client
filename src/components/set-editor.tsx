...
export function SetEditor({
  ...
  const permissive =
    unrestricted || /hackmons|almostanyability|customgame|balancedhackmons|mixandmega/.test(formatId);
  const generation = genForFormat(formatId, permissive);
  const defaultLevel = /vgc|bss|battlestadium|doublesflat/.test(formatId)
    ? 50
    : /lc$/.test(formatId)
      ? 5
      : 100;
  ...
  useEffect(() => {
    ...
    void generation.learnsets
      .learnable(species.id)
      .then(data => {
      ...
      .catch(() => {
        /* All moves remain available when learnset loading fails. */
      });
    return () => {
      cancelled = true;
    };
  ...
  const abilityOptions = useMemo(() => {
    ...
    if (permissive && generation)
      return [
        { value: '', label: 'No ability' },
        ...[...generation.abilities].map(entry => ({ value: entry.name, label: entry.name })),
      ];
    if (!species) return [];
  ...
  const moveOptions = useMemo(() => {
    ...
    return [
      { value: '', label: 'Clear move', group: 'Edit', description: '' },
      ...[...generation.moves]
  ...
  const evTotal = STAT_KEYS.reduce(
    (total, key) => total + (set.evs?.[key] ?? (generationNumber <= 2 ? 252 : 0)),
    0,
  );
  ...
  const itemValue = set.item
    ? ready
      ? (getItem(set.item, generationNumber)?.name ?? set.item)
      : set.item
    : '';
  const abilityValue = set.ability
    ? ready
      ? (getAbility(set.ability, generationNumber)?.name ?? set.ability)
      : set.ability
    : '';
  ...
  if (!ready) {
    return (
      <p className="set-editor-loading" role="status">
        {dexError || 'Loading Pokédex…'}
        {dexError && (
          <button
            type="button"
            onClick={() => {
              setDexError('');
              void loadDex().catch(() => setDexError('Pokédex could not load. Retry when online.'));
            }}
          >
            Retry
          </button>
    ...
    <div className="set-editor">
      <header className="set-editor-heading" ref={headingRef}>
        ...
        <SearchableSelect
          ...
          value={set.species}
          onValueChange={value =>
            patch({ species: value, ability: defaultAbilityForSpecies(value, formatId), moves: set.moves })
          }
        />
        {onRemove && (
          <button
            ...
            aria-label={`Remove ${set.species || 'set'}`}
            onClick={onRemove}
          >
            <Trash2 size={15} />
          </button>
      ...
      <div className="set-editor-grid">
        ...
        {(generationNumber >= 3 || set.ability) && (
          <label className="set-field">
            <span>Ability</span>
            <SearchableSelect
              ariaLabel="Ability"
        ...
        {(generationNumber >= 9 || set.teraType) && (
          <label className="set-field">
            <span>Tera type</span>
            <SearchableSelect
              ariaLabel="Tera type"
              emptyLabel="No types match"
              options={ALL_TYPES.filter(type => type !== '???').map(type => ({
                value: type,
                label: type,
                description: '',
              }))}
      ...
      <div className="set-editor-evs">
        <span className="set-section-label">
          EVs{' '}
          <em data-over={generationNumber > 2 && evTotal > MAX_TOTAL_EVS}>
            · {evTotal}/{generationNumber <= 2 ? 1512 : MAX_TOTAL_EVS}
          </em>
        </span>
        <div className="ev-grid">
          {STAT_KEYS.map(key => (
            <label className="ev-field" key={key}>
              ...
              />
              <output aria-label={`${STAT_LABELS[key]} calculated stat`}>
                {calculatedStats?.[key] ?? '—'}
              </output>
            </label>
          ))}
        </div>
        <SpreadSuggestionPanel
          key={`${formatId}:${set.species}`}
          set={set}
          format={formatId}
          onChange={onChange}
        />
        {generationNumber >= 3 && (
          <div className="set-ev-presets">
            <span>Quick spread</span>
            <button
              type="button"
              onClick={() => patch({ evs: { atk: 252, spe: 252, spd: 4 }, nature: 'Jolly' })}
            >
              Fast physical
            </button>
            <button
              type="button"
              onClick={() => patch({ evs: { spa: 252, spe: 252, spd: 4 }, nature: 'Timid' })}
            >
              Fast special
            </button>
            <button type="button" onClick={() => patch({ evs: undefined })}>
              Reset EVs
            </button>
          </div>
        )}
      ...
      <div className="set-editor-grid is-compact">
        <label className="set-field">
          ...
          <input
            ...
            onChange={event =>
              patch({ level: Math.max(1, Math.min(100, Number(event.currentTarget.value) || defaultLevel)) })
            }
          />
        </label>
        <label className="set-field">
