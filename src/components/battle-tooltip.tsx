  ...
  return (
    <span
      ...
      onPointerEnter={event => {
        if (event.pointerType === 'mouse' && !pinned) show();
      }}
      onMouseLeave={hide}
      onFocus={event => {
        if (!pinned && !(event.target instanceof HTMLElement && event.target.closest('.touch-inspect')))
          show();
      }}
      onBlur={hide}
    >
      ...
      <Dialog.Root open={pinned} onOpenChange={setPinned}>
        <Dialog.Trigger asChild>
          <button type="button" className="touch-inspect" aria-label={`Inspect ${label}`} onClick={hide}>
            ⓘ
          </button>
        </Dialog.Trigger>
        <Dialog.Portal>
          ...
          <Dialog.Content
            ref={inspectionRef}
            className="battle-inspection account-dialog"
          ...
          >
            ...
            <Dialog.Description className="visually-hidden">
              Known battle information and move details.
            </Dialog.Description>
            {content()}
...
export function MoveTooltip({ move, format }: { move: BattleChoice; format?: string }) {
  ...
  const accuracy =
    move.accuracy === true || data?.accuracy === true ? 'Never misses' : (move.accuracy ?? data?.accuracy);
  return (
    <>
      <header className="tooltip-heading">
        <strong>{move.name}</strong>
        <span className="type-chip" style={typeStyle(move.type)}>
          {move.type}
        </span>
      </header>
      <dl className="tooltip-facts">
        <div>
          <dt>Category</dt>
          <dd>{move.category ?? data?.category ?? '—'}</dd>
        </div>
        <div>
          <dt>Power</dt>
          <dd>{move.basePower || data?.basePower || '—'}</dd>
        </div>
        <div>
          <dt>Base accuracy</dt>
          <dd>{typeof accuracy === 'number' ? `${accuracy}%` : accuracy || '—'}</dd>
        </div>
        <div>
          <dt>PP</dt>
          <dd>
            {move.ppLeft ?? '—'}/{move.ppMax ?? data?.pp ?? '—'}
          </dd>
        </div>
        {data?.priority ? (
          <div>
            <dt>Priority</dt>
            <dd>{data.priority > 0 ? `+${data.priority}` : data.priority}</dd>
          </div>
        ) : null}
      </dl>
      {(move.description || data?.shortDesc) && (
        <p className="tooltip-desc">{move.description || data?.shortDesc}</p>
      )}
      {move.notes?.map(note => (
        <p className="tooltip-desc" key={note}>
          {note}
        </p>
      ))}
      {move.effectiveness && (
        <p className="tooltip-desc">
          {move.effectiveness} against the displayed opponent. Unknown abilities and items may change the
          result.
        </p>
      )}
    </>
  ...
  return (
    <>
      ...
      {species && (
        <>
          <p className="tooltip-desc">
            {pokemon.stats ? 'Your reported stats (before stat stages)' : 'Species base stats'}
          </p>
          <div className="tooltip-stats">
            {STAT_ORDER.map(stat => (
              <span key={stat}>
                ...
                <b>
                  {pokemon.stats
                    ? ((stat === 'hp' ? pokemon.maxHp : pokemon.stats[stat]) ?? '—')
      ...
      {!pokemon.stats && pokemon.speedRange && (
        <p className="tooltip-desc">
          Possible unmodified Speed: {pokemon.speedRange[0]}–{pokemon.speedRange[1]}.
        </p>
      )}
      {pokemon.boosts && (
        <p className="tooltip-desc">
          ...
          {Object.entries(pokemon.boosts)
            .map(([stat, stage]) => `${stat.toUpperCase()} ${stage! > 0 ? '+' : ''}${stage}`)
            .join(', ')}
        </p>
      )}
      <p className="tooltip-desc">
      ...
      {!!pokemon.knownMoves?.length && (
        <ul className="known-moves" aria-label="Known moves">
          {pokemon.knownMoves.map(move => (
            <li key={move.name}>
              <strong>{move.name}</strong>{' '}
              <span>
                {move.pp !== undefined
                  ? `${move.pp}/${move.maxpp ?? '?'} PP`
                  : Array.isArray(move.used)
                    ? `${move.used.join('–')} PP used`
                    : `${move.used ?? 0} PP used`}
              </span>
            </li>
          ))}
      ...
      {pokemon.counters?.map(counter => (
        <p className="tooltip-desc" key={counter}>
          {counter}
        </p>
      ))}
    </>
