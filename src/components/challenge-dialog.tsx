...
export function ChallengeDialog() {
  ...
  useEffect(() => {
    const open = (event: Event) => {
      ...
      setRequest(next);
      setFormat(next.format || useArenaStore.getState().selectedFormat);
      setTeamId(useArenaStore.getState().activeTeamId || '');
      setError('');
    };
  ...
  const acceptLabel =
    detail?.acceptLabel && detail.acceptLabel !== 'Accept'
      ? detail.acceptLabel
      : invitation
        ? 'Join battle'
        : 'Accept challenge';
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="account-dialog">
          <div className="dialog-heading">
            <div>
              <Dialog.Title>
                {request?.accept ? (invitation ? 'Join battle' : 'Accept challenge') : 'Challenge'} ·{' '}
                {request?.user}
              </Dialog.Title>
              <Dialog.Description>
                {detail?.message || 'Choose the format and team for this match.'}
              </Dialog.Description>
            </div>
            <Dialog.Close className="icon-button" aria-label="Close challenge">
              <X size={17} />
            </Dialog.Close>
          </div>
          <div className="account-form">
            <label>
              <span>Format</span>
              {request?.accept ? (
                <strong>{entry?.name || format}</strong>
              ) : (
                <FormatSelector
                  value={format}
                  formats={formats.filter(
                    item => item.challengeShow !== false && battleSupport(item.id).supported,
                  )}
            ...
            {needsTeam ? (
              ...
              <p>
                {teamFormat
                  ? 'The server will provide your team.'
                  : 'This seat already has a team. You can join directly.'}
              </p>
            )}
            ...
            <button
              ...
              onClick={() => {
                ...
                if (needsTeam) {
                  ...
                  if (!teamId || !validation.ok) {
                    setError(validation.errors.join(' ') || 'Choose a team.');
                    return;
                  }
                  state.selectTeam(teamId);
                }
                useArenaStore.setState({ lastError: undefined });
                if (request.accept) state.acceptChallenge(request.user);
                else state.sendChallenge(request.user, format);
                const failure = useArenaStore.getState().lastError;
                if (failure) setError(failure);
                else setRequest(undefined);
              }}
            >
              {request?.accept ? acceptLabel : 'Send challenge'}
            </button>
            {request?.accept && (
              <button
                type="button"
