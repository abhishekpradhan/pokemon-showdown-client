...
const gimmicksFor = (moves: BattleChoice[]): Gimmick[] => {
  return [
    moves.some(move => move.canTerastallize) && {
      flag: 'terastallize',
      label: 'Terastallize',
      hint: 'Change your type for the rest of the battle',
    },
    moves.some(move => move.canMegaEvo) && {
      flag: 'mega',
      label: 'Mega Evolve',
      hint: 'Mega Evolve before attacking',
    },
    moves.some(move => move.canZMove) && {
      flag: 'zmove',
      label: 'Z-Move',
      hint: 'Choose an eligible Z-Move',
    },
    moves.some(move => move.canDynamax) && {
      flag: 'dynamax',
      label: 'Dynamax',
      hint: 'Dynamax before attacking',
    },
    moves.some(move => move.canUltraBurst) && {
      flag: 'ultra',
      label: 'Ultra Burst',
      hint: 'Ultra Burst before attacking',
    },
...
export function MoveControls({
  ...
  const choose = (move: BattleChoice) => {
    ...
    const cmd =
      armed && !move.modifier ? move.cmd.replace(/(\|\d+)?$/, match => ` ${armed}${match}`) : move.cmd;
    onChoose({ ...move, cmd });
    setArmed(null);
  };
    ...
    <div className="move-deck-inner">
      {gimmicks.length > 0 && (
        <div className="gimmick-row" role="group" aria-label="Turn modifiers">
          ...
          {armed && (
            <span className="gimmick-hint" role="status">
        ...
        {moves.map(base => {
          ...
          return (
            <TooltipTrigger
              ...
              label={move.name}
              content={() => <MoveTooltip move={move} format={format} />}
            >
              <button
                type="button"
              ...
              >
                <span className="move-headline">
                  ...
                  {move.effectiveness && move.effectiveness !== '1x' && (
                    <em className="move-effect" data-effect={tone}>
                      {move.effectiveness}
                    </em>
                  )}
                </span>
                ...
                <small
                  className="move-pp"
                  data-low={!!move.ppLeft && !!move.ppMax && move.ppLeft <= move.ppMax * 0.25}
                >
                  {move.pp} PP
                </small>
              </button>
