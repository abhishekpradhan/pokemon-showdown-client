...
void loadDex().catch(() => useArenaStore.setState({ sessionNotice: 'Pokémon data could not load. Reconnect and reload to retry.' }));

// The versioned worker preloads local tools and waits before applying an update.
window.addEventListener('load', () => { void registerClientWorker(); });
void loadEngine().catch(() => useArenaStore.setState({ sessionNotice: 'The battle engine could not load. Reload before joining a battle.' }));
