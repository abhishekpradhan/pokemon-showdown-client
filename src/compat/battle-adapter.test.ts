  ...
  it('builds a doubles deck per active slot with protocol-convention targets', () => {
    const doubles: BattleRequest = {
      ...
      active: [
        {
          moves: [
            { move: 'Dazzling Gleam', id: 'dazzlinggleam', pp: 16, maxpp: 16, target: 'allAdjacentFoes' },
          ],
        },
...
describe('battle request regression corpus', () => {
  const pokemon = (name: string, condition = '100/100', extra = {}) => ({
    ident: `p1: ${name}`,
    details: name,
    condition,
    ...extra,
  });
  const singles = (): BattleRequest => ({
    rqid: 10,
    active: [{ moves: [{ move: 'Tackle', target: 'normal' }] }],
    side: { id: 'p1', pokemon: [pokemon('Pikachu', '100/100', { active: true }), pokemon('Bulbasaur')] },
  });
  ...
  it('derives double leads, Illusion order and server preview counts without losing explicit counts', () => {
    ...
    expect(
      normalizeBattleRequest({
        teamPreview: true,
        side: { pokemon: [...side.pokemon, pokemon('Zoroark', '100/100', { baseAbility: 'illusion' })] },
      }).chosenTeamSize,
    ).toBe(4);
    expect(normalizeBattleRequest({ teamPreview: true, side, teamPreviewCount: 2 }).chosenTeamSize).toBe(2);
    expect(
      normalizeBattleRequest({ teamPreview: true, side, chosenTeamSize: 3, teamPreviewCount: 2 })
        .chosenTeamSize,
    ).toBe(3);
  });
  ...
  it('completes scarce replacements in either position and rejects an unnecessary pass', () => {
    const request: BattleRequest = {
      ...
      side: { pokemon: [pokemon('A', '0 fnt'), pokemon('B', '0 fnt'), pokemon('C')] },
    };
    ...
    expect(addBattleChoice(skipped.session, { kind: 'switch', slot: 3 }).command).toBe(
      '/choose pass, switch 3|11',
    );
    expect(
      addBattleChoice(createBattleChoiceSession({ ...request, forceSwitch: [true] }), { kind: 'pass' }).ok,
    ).toBe(false);
  });
  ...
  it('selects only fainted targets for revival, including a fainted active position', () => {
    const request: BattleRequest = {
      ...
      side: {
        pokemon: [
          pokemon('Pawmot', '100/100', { reviving: true }),
          pokemon('Pikachu', '0 fnt'),
          pokemon('Bulbasaur'),
        ],
      },
    };
  ...
  it('automatically skips Commander and fainted non-null active requests', () => {
    const request: BattleRequest = {
      rqid: 13,
      active: [{ moves: [{ move: 'Splash' }] }, { moves: [{ move: 'Protect', target: 'self' }] }],
      side: { pokemon: [pokemon('Tatsugiri', '100/100', { commanding: true }), pokemon('Dondozo')] },
    };
    const session = createBattleChoiceSession(request);
    expect(session.draft.choices).toEqual(['pass']);
    expect(addBattleChoice(session, { kind: 'move', slot: 1, activeIndex: 1 }).command).toBe(
      '/choose pass, move 1|13',
    );
  ...
  it('rejects submitted, stale-position and illegal self-target choices', () => {
    const chosen = addBattleChoice(createBattleChoiceSession(singles()), {
      kind: 'move',
      slot: 1,
      activeIndex: 0,
    });
    ...
    expect(
      addBattleChoice(createBattleChoiceSession(singles()), { kind: 'move', slot: 1, activeIndex: 1 }).ok,
    ).toBe(false);
    expect(
      addBattleChoice(createBattleChoiceSession({ ...singles(), targetable: true }), {
        kind: 'move',
        slot: 1,
        target: -1,
      }).ok,
    ...
    expect(
      addBattleChoice(restoreBattleChoiceSession(singles(), 'move 1'), { kind: 'move', slot: 1 }).ok,
    ).toBe(false);
  ...
  it('honors cancellation restrictions revealed by the last choice', () => {
    ...
    expect(
      addBattleChoice(createBattleChoiceSession(request), { kind: 'switch', slot: 2 }).session.noCancel,
    ).toBe(true);
    request.active![0]!.maybeDisabled = true;
    expect(
      addBattleChoice(createBattleChoiceSession(request), { kind: 'move', slot: 1 }).session.noCancel,
    ).toBe(true);
  });
  ...
  it('uses legal ongoing Max moves even when base moves are disabled', () => {
    ...
    expect(addBattleChoice(createBattleChoiceSession(request), { kind: 'move', slot: 1 }).command).toBe(
      '/choose move 1|10',
    );
  });
  ...
  it('uses transformed targeting and rejects an ineligible Z move', () => {
    const request = singles();
    request.targetable = true;
    request.active = [
      {
        moves: [
          { move: 'Surf', target: 'allAdjacent' },
        ...
        maxMoves: {
          maxMoves: [
            { move: 'Max Geyser', target: 'adjacentFoe' },
            { move: 'Max Strike', target: 'adjacentFoe' },
          ],
        },
      },
    ];
    const deck = buildMoveDeck(request, undefined, 'gen8ou');
    expect(deck[0].canZMove).toBe(false);
    expect(deck[1].zMove?.name).toBe('Breakneck Blitz');
    expect(deck[0].maxMove?.targetOptions).toEqual([1]);
    expect(addBattleChoice(createBattleChoiceSession(request), { kind: 'move', slot: 1, z: true }).ok).toBe(
      false,
    );
    ...
    expect(addBattleChoice(pending.session, { kind: 'move', slot: 1, max: true, target: 1 }).command).toBe(
      '/choose move 1 max +1|10',
    );
  });
  ...
  it('uses effective abilities, grounded states and transformed move types for matchup hints', async () => {
    ...
    request.active = [
      {
        ...
        canDynamax: true,
        maxMoves: { maxMoves: [{ move: 'Max Geyser', type: 'Water' }] },
      },
    ];
    ...
    expect(
      buildMoveDeck(request, ['Electric', 'Ghost'], 'gen8ou', 0, { ...defender, effectiveAbility: '' })[0]
        .effectiveness,
    ).toBe('2x');
    expect(
      buildMoveDeck(request, ['Flying'], 'gen8ou', 0, { ...defender, grounded: true })[0].effectiveness,
    ).toBe('1x');
  });
});
