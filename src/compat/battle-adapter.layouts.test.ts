import { addBattleChoice, availableSwitches, buildMoveDeck, canShiftBattleChoice, createBattleChoiceSession, isBattleRequest, moveTargetOptions, normalizeBattleRequest, type BattleRequest, type BattleSideID } from './battle-adapter';

const roster = (id: BattleSideID, count = 6) => ({ id, name: id, pokemon: Array.from({ length: count }, (_, index) => ({
  ident: `${id}: Pokémon${index + 1}`, details: 'Pikachu', condition: '200/200', active: index === 0, moves: ['tackle'],
})) });
const active = { moves: [{ move: 'Tackle', target: 'normal', pp: 20, maxpp: 35 }, { move: 'Protect', target: 'self', pp: 10, maxpp: 16 }] };
const request = (gameType: string, id: BattleSideID = 'p1'): BattleRequest => ({ gameType, rqid: 42, side: roster(id), active: Array.from({ length: gameType === 'triples' ? 3 : 1 }, () => active) });
...
describe('server field-location rules', () => {
  ...
  it.each(['p1', 'p2', 'p3', 'p4'] as const)('targets all other FFA owners from %s, including the negative opponent', side => {
  ...
  it('controls one multi roster while using two locations on each field half', () => {
    ...
    expect(addBattleChoice(createBattleChoiceSession(input), { kind: 'move', slot: 1, target: -1 }).command).toBe('/choose move 1 -1|42');
    expect(addBattleChoice(createBattleChoiceSession(input), { kind: 'move', slot: 1, target: -2 }).ok).toBe(false);
    expect(addBattleChoice(createBattleChoiceSession(input), { kind: 'switch', slot: 2 }).command).toBe('/choose switch 2|42');
  });
  it('collects all triples choices and allows shifts only from live edge positions', () => {
    ...
    expect(canShiftBattleChoice(session)).toBe(true);
    expect(addBattleChoice(session, { kind: 'move', slot: 1, target: 1 }).command).toBe('/choose shift, move 2, move 1 +1|42');
  ...
  it('skips fainted triples slots and handles scarce replacement choices', () => {
    ...
    expect(addBattleChoice(session, { kind: 'switch', slot: 5 }).command).toBe('/choose switch 4, pass, switch 5|42');
  });
  it('previews triples leads and obeys four-player selected-team counts', () => {
    expect(normalizeBattleRequest({ gameType: 'triples', teamPreview: true, side: roster('p1') }).chosenTeamSize).toBe(3);
    const input = { gameType: 'freeforall', teamPreview: true, chosenTeamSize: 3, side: roster('p4'), rqid: 9 };
    let session = createBattleChoiceSession(input);
    ...
    expect(normalizeBattleRequest({ gameType: 'multi', teamPreview: true, side: roster('p3') }).chosenTeamSize).toBe(1);
  });
});
