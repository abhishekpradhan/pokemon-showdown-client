...
const emptyForm = (slot: number, room = roomId) => `<form data-submitsend="/msgroom ${room},/invitebattle {username}, p${slot}"><label>Player ${slot}: <input name="username" class="textbox" placeholder="Username" /></label> <button class="button" type="submit">Add Player</button></form>`;
const invitedForm = '<form data-submitsend="/msgroom battle-gen9multi-1,/uninvitebattle rosa"><label>Player 3: <strong>rosa</strong> (invited) <button type="submit">Uninvite</button></label></form>';
const pm = (message: string) => useArenaStore.getState().handleFrame(parsePsFrame(`|pm|+ArenaAlice| CodexTester|${message}`));
beforeAll(async () => { await loadEngine(); });
beforeEach(() => useArenaStore.setState({ username: 'CodexTester', named: true, connection: 'connected', rooms: {}, challenges: { from: {}, to: null }, lastError: undefined,
  protocol: { send: vi.fn(() => true) } as unknown as typeof initial.protocol,
  formats: [{ id: 'gen9freeforall', name: 'Free-for-all', team: true }, { id: 'gen9multirandombattle', name: 'Multi Random Battle', team: false }] }));
afterEach(() => { useArenaStore.setState(initial, true); vi.restoreAllMocks(); });

it('preserves all five PM fields without inventing a command or dropping a supplied-team signal', () => {
  expect(parseChallengeDetails("/challenge gen9freeforall||You're invited to join a battle (with Alice, Bob)|Join now|Decline")).toEqual({ format: 'gen9freeforall', teambuilderFormat: '', message: "You're invited to join a battle (with Alice, Bob)", acceptLabel: 'Join now', rejectLabel: 'Decline' });
  ...
  expect(parseChallengeDetails(`/challenge gen9ou||${'m'.repeat(1500)}|${'a'.repeat(150)}|${'r'.repeat(150)}`)).toMatchObject({ message: 'm'.repeat(1000), acceptLabel: 'a'.repeat(120), rejectLabel: 'r'.repeat(120) });
});
...
it('validates and sends the requested teambuilder format before accepting a newly assigned seat', () => {
  ...
  pm("/challenge gen9freeforall@@@maxteamsize=3|gen9freeforall|You're invited to join a battle (with Alice, Bob)||");
  useArenaStore.getState().acceptChallenge('arenaalice');
  expect(validate).toHaveBeenCalledWith('chosen-team', 'gen9freeforall');
  expect(vi.mocked(useArenaStore.getState().protocol.send).mock.calls.map(call => call[0])).toEqual([expect.stringMatching(/^\/utm /), '/accept arenaalice']);
});
...
it('only enables schema-matching seat actions scoped to the current battle', () => {
  expect(parseBattleInvitations(emptyForm(3) + emptyForm(4) + invitedForm, roomId)).toEqual([{ slot: 'p3', canInvite: true }, { slot: 'p4', canInvite: true }]);
  ...
  expect(parseBattleInvitations(`<img src="https://invalid.example/track"><iframe src="https://invalid.example/embed"></iframe>${emptyForm(3)}`, roomId)).toEqual([{ slot: 'p3', canInvite: true }]);
});
