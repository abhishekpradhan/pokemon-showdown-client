...
export function parseLadderData(data: unknown): LadderRow[] {
  if (!data || typeof data !== 'object' || !('toplist' in data) || !Array.isArray(data.toplist))
    throw new Error('The server returned an unexpected ladder response.');
  return data.toplist
    .filter(
      (row): row is Record<string, unknown> =>
        !!row && typeof row === 'object' && typeof row.username === 'string',
    )
    .slice(0, 5000)
    .map((row, index) => ({
      ...
      elo: number(row.elo),
      gxe: number(row.gxe),
      glicko: number(row.rpr ?? row.r),
      deviation: number(row.rprd ?? row.rd),
      coil: number(row.coil),
      record:
        row.w !== undefined || row.l !== undefined
          ? `${number(row.w) || 0}–${number(row.l) || 0}${row.t ? `–${number(row.t) || 0}` : ''}`
          : undefined,
    }));
}
...
export function parseLocalLadderHtml(html: string): LadderRow[] {
  ...
  for (const tr of template.content.querySelectorAll('tr')) {
    ...
    rows.push({
      ...
      username: cells[1],
      elo: number(cells[2]),
      gxe: number(cells[3]?.replace('%', '')),
      glicko: rating ? Number(rating[1]) : number(cells[4]),
      deviation: rating ? Number(rating[2]) : undefined,
      coil: number(cells[5]),
    });
  }
...
function requestServerLadderNow(
  protocol: Pick<ProtocolClient, 'send' | 'subscribe'>,
  format: string,
  prefix: string,
  signal: AbortSignal,
): Promise<LadderRow[]> {
  return new Promise((resolve, reject) => {
    let done = false;
    let unsubscribe = () => {};
    const finish = (error?: Error, rows?: LadderRow[]) => {
      if (done) return;
      done = true;
      clearTimeout(timer);
      unsubscribe();
    ...
    const timer = window.setTimeout(
      () => finish(new Error('The server did not answer the ladder request. Retry when connected.')),
      15_000,
    );
    unsubscribe = protocol.subscribe(event => {
      ...
      for (const line of event.frame.lines) {
        ...
        try {
          ...
          finish(error instanceof Error ? error : new Error('Invalid ladder response.'));
        }
      }
    });
    if (signal.aborted) {
    ...
    if (protocol.send(`/cmd laddertop ${toId(format)}${prefix ? ` ,${toId(prefix)}` : ''}`) === false)
      finish(new Error('The connection closed before the ladder request could be sent.'));
  });
}
...
export function requestServerLadder(
  protocol: Pick<ProtocolClient, 'send' | 'subscribe'>,
  format: string,
  prefix: string,
  signal: AbortSignal,
...
export async function requestPublicLadder(
  host: string,
  format: string,
  user: string,
  exact: boolean,
  ...
  if (exact && user) {
    if (
      ...
      throw new Error('No player ratings returned.');
    const rating = (data.ratings as Record<string, unknown>)[format];
    if (!rating || typeof rating !== 'object') return [];
    return parseLadderData({
      toplist: [{ ...rating, username: 'username' in data ? data.username : user }],
    }).map(row => ({ ...row, rank: 0 }));
  }
