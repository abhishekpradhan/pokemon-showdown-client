...
export function unpackTeam(packed: PackedTeam): TeamSet[] {
  ...
  return packed.trim().split(']').filter(Boolean).map(chunk => {
    ...
    const species = getSpecies(parts[1] || parts[0])?.name || displayName(parts[1] || parts[0] || 'Pokemon');
    const abilityCode = parts[3] || '0';
    const ability = /^[01HS]$/.test(abilityCode) ? getSpecies(species)?.abilities[abilityCode as '0'] || abilityCode : getAbility(abilityCode)?.name || displayName(abilityCode);
...
export function importTeams(text: string, format = 'gen9ou'): StoredTeam[] {
  ...
  for (const block of blocks) {
    ...
    teams.push({ id: `${toId(name)}-${Date.now()}-${teams.length}`, name, folder, format: teamFormat, sets, packed: packTeam(sets), updatedAt: Date.now() });
  }
  return teams;
}
...
export function validateTeamSets(sets: TeamSet[], format = 'gen9ou'): TeamValidationResult {
  ...
  sets.forEach((set, index) => {
    ...
    if (generation > 2 && evTotal > MAX_EV_TOTAL) warnings.push(`${label} has ${evTotal} EVs; the limit is ${MAX_EV_TOTAL}.`);
    ...
    if (dexReady) {
      ...
      if (set.ability && !getAbility(set.ability, generation)) warnings.push(`${label}: unknown ability "${set.ability}".`);
...
export type TeamDraft = { key: string; teamId?: string; baseUpdatedAt?: number; name: string; format: string; folder: string; sets: TeamSet[]; updatedAt: number };
...
const stringFields = ['name', 'item', 'ability', 'nature', 'gender', 'teraType', 'hpType', 'pokeball'] as const;

export const createTeamId = () => globalThis.crypto?.randomUUID?.() || `team-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
export function parseTeamSet(value: unknown): TeamSet | undefined {
  if (!isRecord(value) || typeof value.species !== 'string' || !Array.isArray(value.moves) || !value.moves.every(move => typeof move === 'string')) return;
  ...
  for (const field of numericFields) {
    if (value[field] !== undefined && (typeof value[field] !== 'number' || !Number.isFinite(value[field]))) return;
...
function parseStoredTeam(value: unknown): StoredTeam | undefined {
  if (!isRecord(value) || typeof value.id !== 'string' || typeof value.name !== 'string' || typeof value.format !== 'string' || !Array.isArray(value.sets)) return;
  const sets = value.sets.map(parseTeamSet);
  if (sets.some(set => !set)) return;
  return { id: value.id, name: value.name, format: value.format, folder: typeof value.folder === 'string' ? value.folder : '', sets: sets as TeamSet[], packed: packTeam(sets as TeamSet[]), updatedAt: typeof value.updatedAt === 'number' && Number.isFinite(value.updatedAt) ? value.updatedAt : 0 };
}

export function parseLibrary(raw: string): { teams: StoredTeam[]; rejected: unknown[] } {
  const data: unknown = JSON.parse(raw);
  const records = Array.isArray(data) ? data : isRecord(data) && data.version === 2 && Array.isArray(data.teams) ? data.teams : null;
  ...
  for (const record of records) {
    ...
    else { teams.push(team); ids.add(team.id); }
  }
  return { teams, rejected };
}

export function hasStoredTeamLibrary() {
  try { return window.localStorage.getItem(TEAM_STORAGE_KEY) !== null; } catch { return true; }
}
...
export function loadStoredTeams(): StoredTeam[] {
  ...
  try {
    ...
    if (result.rejected.length) {
      ...
      try { window.localStorage.setItem(RECOVERY_KEY, raw); } catch { /* Original remains untouched. */ }
    }
    return result.teams;
  } catch {
    storageProblem = 'Team storage could not be read. Export recovery data before replacing it.';
    if (librarySnapshot) { try { window.localStorage.setItem(RECOVERY_KEY, librarySnapshot); } catch { /* Original stays available until export. */ } }
    return [];
  }
}
...
export function teamRecoveryData() {
  try { return window.localStorage.getItem(RECOVERY_KEY) || window.localStorage.getItem(TEAM_STORAGE_KEY) || ''; } catch { return ''; }
}
...
export function saveStoredTeams(teams: StoredTeam[]): StorageResult {
  ...
  try {
    ...
    if (librarySnapshot !== undefined && current !== librarySnapshot) return { ok: false, conflict: true, error: 'Teams changed in another tab. Reload the library, then save your preserved draft.' };
    ...
    return { ok: false, error: 'Could not save to browser storage. Your edits remain open; export a backup or free storage and retry.' };
  }
}
...
export function loadTeamDraft(key: string, track = true): TeamDraft | undefined {
  try {
    ...
    if (!isRecord(value) || value.key !== key || typeof value.name !== 'string' || typeof value.format !== 'string' || !Array.isArray(value.sets)) return;
    const sets = value.sets.map(parseTeamSet);
    if (sets.some(set => !set)) return;
    return { key, teamId: typeof value.teamId === 'string' ? value.teamId : undefined, baseUpdatedAt: typeof value.baseUpdatedAt === 'number' ? value.baseUpdatedAt : undefined, name: value.name, format: value.format, folder: typeof value.folder === 'string' ? value.folder : '', sets: sets as TeamSet[], updatedAt: typeof value.updatedAt === 'number' ? value.updatedAt : 0 };
  } catch { return; }
}

export function listTeamDrafts(): TeamDraft[] {
  try {
    return Array.from({ length: localStorage.length }, (_, i) => localStorage.key(i)).filter((key): key is string => !!key?.startsWith(DRAFT_PREFIX)).map(key => loadTeamDraft(key.slice(DRAFT_PREFIX.length), false)).filter((draft): draft is TeamDraft => !!draft).sort((a, b) => b.updatedAt - a.updatedAt);
  } catch { return []; }
}
...
export function saveTeamDraft(draft: TeamDraft): StorageResult {
  try {
    ...
    if (previous !== undefined && current !== previous) return { ok: false, conflict: true, error: 'This draft changed in another tab. Export your edits or save a copy before reloading.' };
    const raw = JSON.stringify(draft);
    window.localStorage.setItem(key, raw);
    draftSnapshots.set(draft.key, raw);
    return { ok: true };
  } catch { return { ok: false, error: 'Draft recovery could not be saved. Keep this page open and export your edits.' }; }
}

export function removeTeamDraft(key: string) {
  try { window.localStorage.removeItem(DRAFT_PREFIX + key); draftSnapshots.delete(key); } catch { /* Library save already succeeded. */ }
}

export function exportTeams(teams: StoredTeam[]): string {
  return teams.map(team => `=== [${team.format}] ${team.folder ? `${team.folder}/` : ''}${team.name} ===\n\n${exportTeam(team.sets)}`).join('\n\n');
}
...
export function importTeamLibrary(text: string, format = 'gen9ou'): StoredTeam[] {
  ...
  if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
    ...
    if (rejected.length) throw new Error('This backup contains invalid records. Keep the original and export recovery data.');
  ...
  if (!trimmed.startsWith('===') && trimmed.split('\n').every(line => line.includes(']') && line.slice(0, line.indexOf(']')).split('|').length === 2)) {
    return trimmed.split('\n').map(line => {
      ...
      return { id: createTeamId(), name: slash >= 0 ? fullName.slice(slash + 1) : fullName, folder: slash >= 0 ? fullName.slice(0, slash) : '', format: teamFormat || format, sets, packed: packTeam(sets), updatedAt: Date.now() };
    });
  }
