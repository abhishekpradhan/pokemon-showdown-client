...
export function sanitizeProtocolLog(raw: string): string {
  return raw.split(/\r?\n/).map(line => {
    ...
    if (command && ['challstr', 'pm', 'c', 'c:', 'chat', 'raw', 'html', 'uhtml', 'uhtmlchange', 'request', 'popup', 'queryresponse', 'updateuser', 'updatechallenges', 'users', 'player', 'j', 'J', 'l', 'L', 'n', 'N'].includes(command)) return `|${command}|[private payload omitted]`;
    if (/\/(?:utm|trn|pm|msg|challenge|accept|reject|savereplay|avatar)\b/i.test(line)) return '[private command omitted]';
