  ...
  persist(
    set => ({
      ...
      setPreference: (key, value) =>
        set(
          key === 'volume' && typeof value === 'number'
            ? {
                volume: clampVolume(value),
                effectsVolume: clampVolume(value),
                notificationVolume: clampVolume(value),
              }
            : { [key]: value },
        ),
    ...
    }),
    {
      ...
      merge: (stored, current) => {
        ...
        if (typeof data.volume === 'number' && Number.isFinite(data.volume))
          safe.volume = Math.max(0, Math.min(100, data.volume));
        ...
        for (const key of [
          ...
          if (Array.isArray(data[key]))
            safe[key] = data[key]
              .filter((entry): entry is string => typeof entry === 'string' && entry.length <= 120)
              .slice(0, 100);
        }
        return safe;
      },
