...
beforeEach(() => {
  ...
  useArenaStore.setState({ ...initial, username: 'Alice', named: true, connection: 'connected', lastError: undefined,
    formats: [{ id: 'gen9ou', name: '[Gen 9] OU', team: true }], activeTeamId: 'team', activeTeam: packTeam(sets), teams: [{ id: 'team', name: 'Team', sets, packed: packTeam(sets), format: 'gen9ou', updatedAt: 1 }],
    challenges: { from: { Bob: 'gen9ou' }, to: { to: 'Carol', format: 'gen9ou' } }, protocol: { send } as unknown as typeof initial.protocol });
  send.mockReset(); send.mockReturnValue(true);
});
...
it('never accepts a challenge after team upload fails', () => {
  ...
  expect(send).toHaveBeenCalledTimes(1); expect(send.mock.calls[0][0]).toMatch(/^\/utm /);
...
it('preserves incoming and outgoing challenges after failed rejection or cancellation', () => {
  ...
  useArenaStore.getState().rejectChallenge('Bob'); useArenaStore.getState().cancelChallenge();
  expect(useArenaStore.getState().challenges).toEqual({ from: { Bob: 'gen9ou' }, to: { to: 'Carol', format: 'gen9ou' } });
