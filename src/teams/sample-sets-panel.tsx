...
function SampleSetBrowser({ set, format, onChange }: Props) {
  ...
  useEffect(() => {
    ...
    void fetchSampleSets(format, set.species, controller.signal)
      .then(samples => {
          ...
          setResult({
            ...
            error:
              error instanceof Error
                ? error.message
                : 'Sample sets could not load. Check your connection and retry.',
          });
      });
  ...
  try {
    sourceUrl = sampleSetUrl(format);
  } catch {
    /* The loading state explains unsupported format IDs. */
  }

  return (
    <div className="sample-set-browser">
      ...
      {current?.error && (
        <div className="set-suggestion-error" role="alert">
          <p>{current.error}</p>
      ...
      {current && !current.error && !current.samples.length && (
        <p role="status">
          No published sample sets for {set.species} in {format}. You can still edit or import a set manually.
        </p>
      )}
      {sample && preview && (
        <>
          <SearchableSelect
            ...
            options={current!.samples.map(entry => ({
              value: entry.id,
              label: entry.name,
              group: entry.source === 'analysis' ? 'Smogon analysis' : 'Showdown usage',
              description: entry.source === 'analysis' ? 'Analysis sample' : 'Usage-derived sample',
            }))}
            value={sample.id}
            onValueChange={setSelected}
          />
          <p className="set-suggestion-note">
            Preview: supplied sample fields replace this Pokémon’s corresponding fields. Nickname and other
            personal details are retained.
          </p>
          <pre className="sample-set-preview" aria-label="Sample set preview" tabIndex={0}>
            {exportTeam([preview])}
          </pre>
          <button
            type="button"
            className="secondary-action"
            disabled={JSON.stringify(preview) === JSON.stringify(set)}
            onClick={() => {
...
export function SampleSetsPanel(props: Props) {
  ...
  return (
    <section className="set-sample-section" aria-label="Competitive sample sets">
      <button
        type="button"
        className="secondary-action"
        aria-expanded={open}
        disabled={!props.set.species}
