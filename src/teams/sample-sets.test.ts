...
import { applySampleSet, fetchSampleSets, MAX_SAMPLE_BYTES, parseSampleCatalog, sampleSetUrl, SAMPLE_TIMEOUT_MS } from './sample-sets';

const dataset = JSON.stringify({
  dex: { Clefable: { 'Calm Mind': { moves: ['Calm Mind', 'Moonblast', 'Moonlight', 'Knock Off'], ability: 'Magic Guard', item: 'Leftovers', nature: 'Bold', ivs: { atk: 0 }, evs: { hp: 252, def: 252, spd: 4 }, teraType: 'Water' } } },
...
it('keeps analysis and usage provenance and zero-valued details', () => {
  ...
  expect(applySampleSet({ species: 'clefable', name: 'My partner', moves: ['Tackle'], gender: 'F', shiny: true, happiness: 0, pokeball: 'Cherish Ball', hpType: 'Ice' }, entries[0])).toMatchObject({
    species: 'clefable', name: 'My partner', moves: ['Calm Mind', 'Moonblast', 'Moonlight', 'Knock Off'], gender: 'F', shiny: true, happiness: 0, pokeball: 'Cherish Ball', hpType: 'Ice', ivs: { atk: 0 }, nature: 'Bold',
  });
...
it('rejects unsafe paths and malformed sample fields without importing arbitrary keys', () => {
  for (const format of ['gen9', '../gen9ou', 'gen9ou?x=1', 'gen10ou', 'https://other.test']) expect(() => sampleSetUrl(format)).toThrow();
  const catalog = parseSampleCatalog('{"dex":{"Clefable":{"bad":{"moves":["Moonblast\\n/forfeit"]},"bad ev":{"moves":["Moonblast"],"evs":{"hp":999}},"safe":{"moves":["Moonblast"],"__proto__":{"polluted":true},"name":"overwrite"}}}}');
...
it('uses the official endpoint without credentials and caches immutable parsed samples', async () => {
  ...
  expect(fetcher).toHaveBeenCalledWith('https://play.pokemonshowdown.com/data/sets/gen9ou.json', expect.objectContaining({ credentials: 'omit', referrerPolicy: 'no-referrer', redirect: 'error' }));
});

it('distinguishes unpublished formats from failures that can retry', async () => {
  const fetcher = vi.fn().mockResolvedValueOnce(new Response('', { status: 404 })).mockResolvedValueOnce(new Response('', { status: 503 })).mockResolvedValueOnce(new Response(dataset));
  vi.stubGlobal('fetch', fetcher);
...
it('enforces streamed byte limits without waiting for a stuck cancel operation', async () => {
  ...
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(new ReadableStream({ start(controller) { controller.enqueue(new Uint8Array(MAX_SAMPLE_BYTES + 1)); }, cancel }))));
...
it('bounds stalled fetches and body reads, and cancels explicitly dismissed loads', async () => {
  ...
  const pending = expect(fetchSampleSets('gen9nu', 'Clefable', new AbortController().signal)).rejects.toThrow('timed out');
  await vi.advanceTimersByTimeAsync(SAMPLE_TIMEOUT_MS);
  await pending;
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(new ReadableStream({ cancel: () => new Promise<void>(() => {}) }))));
  const body = expect(fetchSampleSets('gen9pu', 'Clefable', new AbortController().signal)).rejects.toThrow('timed out');
  await vi.advanceTimersByTimeAsync(SAMPLE_TIMEOUT_MS);
  await body;
  const controller = new AbortController();
  const dismissed = expect(fetchSampleSets('gen9lc', 'Clefable', controller.signal)).rejects.toMatchObject({ name: 'AbortError' });
  controller.abort();
  await dismissed;
});
