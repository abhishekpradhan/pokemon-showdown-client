...
const dragapult: TeamSet = { species: 'Dragapult', item: 'choicespecs', ability: 'infiltrator', moves: ['Shadow Ball', 'Draco Meteor', 'Flamethrower', 'U-turn'] };
...
it('infers a special Choice role from moves and canonicalizes packed IDs', async () => {
  ...
  expect(suggestion).toMatchObject({ available: true, role: 'Fast Specs', evs: { spa: 252, spe: 252 }, nature: 'Timid' });
  ...
  const current = { ...dragapult, name: 'Specialist', ivs: { atk: 0, spe: 17 }, happiness: 0, teraType: 'Stellar', gender: 'F', shiny: true };
  expect(applySpreadSuggestion(current, suggestion)).toMatchObject({ name: 'Specialist', ivs: { atk: 0, spe: 17 }, happiness: 0, teraType: 'Stellar', gender: 'F', shiny: true });
...
it('distinguishes defensive, physical, and Trick Room roles instead of always maximizing speed', async () => {
  const defensive = await suggestSpread({ species: 'Blissey', item: 'Heavy-Duty Boots', ability: 'Natural Cure', moves: ['Soft-Boiled', 'Seismic Toss', 'Thunder Wave', 'Stealth Rock'] }, 'gen9ou');
  expect(defensive).toMatchObject({ available: true, role: 'Physically Defensive', nature: 'Bold' });
  const physical = await suggestSpread({ species: 'Garchomp', item: 'Choice Band', moves: ['Earthquake', 'Outrage', 'Stone Edge', 'Fire Fang'] }, 'gen4ou');
  expect(physical).toMatchObject({ available: true, role: 'Fast Band', evs: { atk: 252, spe: 252 }, nature: 'Jolly' });
  const slow = await suggestSpread({ species: 'Bronzong', item: 'Leftovers', moves: ['Trick Room', 'Gyro Ball', 'Earthquake', 'Stealth Rock'] }, 'gen4ou');
...
it('handles early-generation stat experience and low-level rounding without applying a nature', async () => {
  const old = await suggestSpread({ species: 'Alakazam', moves: ['Psychic', 'Recover', 'Thunder Wave', 'Reflect'] }, 'gen1ou');
  ...
  const low = await suggestSpread({ species: 'Abra', moves: ['Psychic', 'Shadow Ball', 'Energy Ball', 'Protect'], item: 'Focus Sash' }, 'gen7lc');
...
it('does not guess unknown data or unsupported stat systems and permits the Ditto exception', async () => {
  expect(await suggestSpread({ species: 'Pikachu', moves: ['Thunderbolt'] }, 'gen9ou')).toMatchObject({ available: false, reason: expect.stringContaining('four moves') });
  expect(await suggestSpread({ species: 'Pikachu', moves: ['Custom Attack'] }, 'gen9customgame')).toMatchObject({ available: false, reason: expect.stringContaining('unknown') });
  ...
  expect(await suggestSpread({ species: 'Ditto', moves: ['Transform'], ability: 'Imposter' }, 'gen9ou')).toMatchObject({ available: true, role: 'Physically Defensive' });
});
