...
export function SetIVFields({
  set,
  generation,
  calculatedStats,
  onChange,
}: Props & { calculatedStats: StatTable | null }) {
    ...
    <details className="set-details set-iv-details">
      ...
      <div className="ev-grid">
        {(Object.keys(labels) as Array<keyof StatTable>).map(stat => {
          ...
          return (
            <label className="ev-field" key={stat}>
              ...
              <input
                ...
                onChange={event => {
                  const value = Math.max(
                    0,
                    Math.min(oldGeneration ? 15 : 31, Math.trunc(Number(event.currentTarget.value) || 0)),
                  );
      ...
      <div className="button-row">
        <button
          ...
          onClick={() => patch({ ivs: { ...set.ivs, atk: 0 } })}
        >
          Minimum Attack
        </button>
        <button
          type="button"
          className="secondary-action"
          onClick={() => patch({ ivs: { ...set.ivs, spe: 0 } })}
        >
          Minimum Speed
        </button>
        <button type="button" className="secondary-action" onClick={() => patch({ ivs: undefined })}>
          Reset IVs
        </button>
      </div>
    </details>
      ...
      <div className="set-editor-grid is-compact">
        <label className="set-field">
          ...
          <select
            aria-label="Gender"
            value={set.gender || ''}
            onChange={event => patch({ gender: event.currentTarget.value || undefined })}
          >
        ...
        <label className="set-field">
          ...
          <input
            ...
            type="checkbox"
            checked={!!set.shiny}
            onChange={event => patch({ shiny: event.currentTarget.checked })}
          />
        </label>
        <label className="set-field">
          <span>Happiness</span>
          <input
            ...
            onChange={event =>
              patch({ happiness: Math.max(0, Math.min(255, Number(event.currentTarget.value) || 0)) })
            }
          />
        </label>
        <label className="set-field">
          ...
          <select
            aria-label="Hidden Power type"
            value={set.hpType || ''}
            onChange={event => patch({ hpType: event.currentTarget.value || undefined })}
          >
            <option value="">Default</option>
            {ALL_TYPES.filter(type => !['???', 'Normal', 'Fairy', 'Stellar'].includes(type)).map(type => (
        ...
        <label className="set-field">
          ...
          <input
            value={set.pokeball || ''}
            placeholder="Default"
            onChange={event => patch({ pokeball: event.currentTarget.value || undefined })}
          />
        </label>
          ...
          <>
            <label className="set-field">
              ...
              <input
                ...
                min={0}
                max={10}
                value={set.dynamaxLevel ?? 10}
                onChange={event =>
                  patch({ dynamaxLevel: Math.max(0, Math.min(10, Number(event.currentTarget.value) || 0)) })
                }
              />
            </label>
            <label className="set-field">
              ...
              <input
                ...
                onChange={event => patch({ gigantamax: event.currentTarget.checked })}
              />
            </label>
          </>
        )}
