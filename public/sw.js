...
const BUILD = /* @arena-manifest */ {
  ...
  assets: ['/', '/favicon.svg', '/manifest.webmanifest', '/icon-512.png'],
};
const PREFIX = 'arena-build-';
const CACHE = `${PREFIX}${BUILD.revision}`;
const CORE = new Set(BUILD.assets);
const isAppPath = pathname =>
  /^\/(?:index\.html|teambuilder|settings|rooms|battles|ladder|replays)?$/.test(pathname) ||
  /^\/(?:room|battle)\/[^/]+$/.test(pathname) ||
  /^\/(?:battle-[a-z0-9-]+|pm-[a-z0-9]+|lobby)$/.test(pathname);
  ...
  event.waitUntil(
    (async () => {
      ...
      try {
        ...
        if (!shell.ok || !html.includes(`name="arena-build" content="${BUILD.revision}"`))
          throw new Error('Deployment changed during install');
        ...
        for (const path of resources) {
          ...
          if (
            !response ||
            (/\.js$/.test(path) && !/(java|ecma)script/i.test(type)) ||
            (/\.css$/.test(path) && !/text\/css/i.test(type))
          ) {
    ...
    (async () => {
      ...
      await Promise.all(
        keys
          .filter(
            key =>
              (key.startsWith(PREFIX) && key !== CACHE && key !== previous) ||
              /^arena-(shell|assets)-/.test(key),
          )
