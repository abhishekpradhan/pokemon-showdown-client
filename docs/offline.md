...
Automated worker tests cover callback/API exclusion, matching installation, quota failure, offline editor resources, previous-version retention, completeness probes and cleanup. Production browser tests cover an online callback followed by offline species/move selection, saving and reload; a native worker test verifies rejected incomplete installation, consent before activation and two successive updates retaining only the current/previous caches. Release checks should additionally cover an actual prior-to-next hosted deployment upgrade and a real mobile installation.
