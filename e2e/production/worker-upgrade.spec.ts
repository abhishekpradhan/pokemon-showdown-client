...
test('native worker updates wait, reject incomplete installs and retain exactly one prior release', async ({ page, context }) => {
  let revision = 'first';
  ...
  const server = createServer((request, response) => {
    ...
    if (request.url === '/sw.js') {
      ...
      response.end(template.replace(/const BUILD = \/\* @arena-manifest \*\/ .*;/, `const BUILD = ${JSON.stringify({ revision, assets: ['/', `/assets/${revision}.js`] })};`));
      ...
      response.end(`<html><head><meta name="arena-build" content="${revision}"></head><body>${revision}</body></html>`);
    }
  });
  ...
  try {
      ...
      await new Promise<void>((resolve, reject) => {
        registration.addEventListener('updatefound', () => {
          ...
          worker.addEventListener('statechange', () => {
            ...
            if (worker.state === 'installed') reject(new Error('Incomplete resources must reject installation'));
          });
        }, { once: true });
    ...
    for (const next of ['second', 'third']) {
      ...
      await page.evaluate(async () => {
        const registration = (await navigator.serviceWorker.getRegistration())!;
        const activated = new Promise<void>(resolve => navigator.serviceWorker.addEventListener('controllerchange', () => resolve(), { once: true }));
    ...
    expect(await page.evaluate(() => caches.keys())).toEqual(['unrelated-app', 'arena-build-second', 'arena-build-third']);
