...
test('deployed handlers reject unsafe inputs and expose production headers', async ({ request }) => {
  ...
  expect((await request.post('/api/action', { data: '{}', headers: { 'Content-Type': 'application/json' } })).status()).toBe(415);
  expect((await request.post('/api/action', { form: { act: 'login' } })).status()).toBe(400);
  const crossOrigin = await request.post('/api/replay', { form: { id: 'test', log: '|turn|1' }, headers: { Origin: 'https://another.example' } });
...
test('OAuth callback stays separate under CSP and cannot replace the offline team editor', async ({ page, context }, testInfo) => {
  const errors: string[] = [];
  ...
  const response = await callback.goto('/oauth.html?state=synthetic-state&assertion=synthetic-assertion&token=synthetic-token&user=Alice');
  ...
  await page.getByRole('option').filter({ has: page.getByText('Thunderbolt', { exact: true }) }).click();
