...
test('an old account profile update keeps the rename dialog pending until the requested identity arrives', async ({
  page,
}) => {
  ...
  await page.evaluate(() => {
    const harness = window as unknown as {
      ...
      releaseAssertion?: () => void;
      requestedNewIdentity?: boolean;
    };
    const socket = harness.__mockPsSockets[0];
    socket.emit('|updateuser| ArenaOld|1|lucas');
    const originalSend = socket.send.bind(socket);
    socket.send = raw => {
      if (raw.includes('/trn ArenaNew,')) {
        harness.requestedNewIdentity = true;
        return;
      }
      originalSend(raw);
    };
    ...
    window.fetch = (input, init) => {
      ...
      if (
        ...
        new URLSearchParams(String(init?.body)).get('userid') === 'arenanew'
      ) {
        return new Promise(resolve => {
          harness.releaseAssertion = () => resolve(new Response('synthetic-assertion'));
        });
      }
  ...
  const emitOld = () =>
    page.evaluate(() =>
      (
        window as unknown as { __mockPsSockets: Array<{ emit: (raw: string) => void }> }
      ).__mockPsSockets[0].emit('|updateuser| ArenaOld@!|1|dawn|{"language":"french"}'),
    );
  await emitOld();
  ...
  await page.waitForFunction(
    () => (window as unknown as { requestedNewIdentity: boolean }).requestedNewIdentity,
  );
  await emitOld();
  await expect(dialog).toBeVisible();
  await expect(dialog.getByRole('button', { name: 'Submitting…', exact: true })).toBeDisabled();
  await page.evaluate(() =>
    (
      window as unknown as { __mockPsSockets: Array<{ emit: (raw: string) => void }> }
    ).__mockPsSockets[0].emit('|updateuser| ArenaNew|1|dawn'),
  );
