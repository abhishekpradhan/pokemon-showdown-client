...
const emit = (page: Page, frame: string) =>
  page.evaluate(
    frame =>
      (
        window as unknown as { __mockPsSockets: Array<{ emit: (value: string) => void }> }
      ).__mockPsSockets[0].emit(frame),
    frame,
  );
...
test('hosts invite both remaining seats, revoke an invitation and reach the started battle', async ({
  page,
}, info) => {
  const unexpectedResources: string[] = [];
  await page.route('https://invitation-resource.invalid/**', route => {
    unexpectedResources.push(route.request().url());
    return route.abort();
  });
  await emit(
    page,
