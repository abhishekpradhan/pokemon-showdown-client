...
async function installAudioProbe(page: Page, musicEnabled: boolean) {
  ...
  await page.addInitScript(({ musicEnabled }) => {
    localStorage.setItem('ps-arena-workspace-v1', JSON.stringify({ version: 0, state: {
      soundEnabled: true, musicEnabled, musicTrack: 'dpp-trainer', musicVolume: 63,
      effectsVolume: 0, notificationVolume: 0, reducedMotion: true,
    } }));
    ...
    class FakeAudio extends EventTarget {
      ...
      preload = '';
      loop = false;
      constructor(src = '') { super(); this.originalSrc = this.src = src; audioRecords.push(this); }
    ...
    class FakeAudioContext {
      ...
      currentTime = 0;
      destination = {};
      constructor() { (window as unknown as { __audioContextCount: number }).__audioContextCount++; }
      resume() { this.state = 'running'; return Promise.resolve(); }
      suspend() { this.state = 'suspended'; return Promise.resolve(); }
      createOscillator() {
        return {
          type: 'sine', frequency: { value: 0 }, onended: undefined as (() => void) | undefined,
          connect: <T,>(target: T) => target, disconnect() {}, start() {},
          stop() { this.onended?.(); },
        };
      }
      createGain() {
        return {
          ...
          connect: <T,>(target: T) => target, disconnect() {},
        };
      }
    }
    Object.assign(window, { Audio: FakeAudio, AudioContext: FakeAudioContext, __audioRecords: audioRecords, __audioContextCount: 0 });
  }, { musicEnabled });
}

const music = (page: Page) => page.evaluate(() => (window as unknown as AudioWindow).__audioRecords
  ...
  .map(({ originalSrc, src, volume, paused, playCalls, pauseCalls, loadCalls, preload, loop }) =>
    ({ originalSrc, src, volume, paused, playCalls, pauseCalls, loadCalls, preload, loop })));
...
const returnToBattle = (page: Page) => page.getByRole('navigation', { name: 'Open sessions' }).getByRole('button', { name: /^Listener v Rival/ }).click();
...
async function openBattle(page: Page) {
  ...
  await emit(page, [
    '|init|battle', '|title|Audio workflow', '|gen|9', '|gametype|singles',
    '|player|p1|Listener', '|player|p2|Rival', '|start',
    '|switch|p1a: Pikachu|Pikachu|100/100', '|switch|p2a: Eevee|Eevee|100/100', '|turn|1',
  ].join('\n'));
...
test('focused battle music waits for a gesture, uses one stream, and follows visibility, navigation, mute and battle end', async ({ page }) => {
  ...
  await expect.poll(() => music(page)).toEqual([expect.objectContaining({
    originalSrc: 'https://play.pokemonshowdown.com/audio/dpp-trainer.mp3',
    paused: false, volume: 0.63, playCalls: 1, preload: 'none', loop: true,
  })]);
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('ps-arena-workspace-v1')!).state.musicVolume)).toBe(63);
  ...
  await page.keyboard.press('Shift');
  await settled(page);
  expect(await music(page)).toEqual([expect.objectContaining({ paused: true, playCalls: ended[0].playCalls })]);
});

test('music stays optional, applies settings on return to battle, and releases the previous track', async ({ page }) => {
  ...
  await returnToBattle(page);
  await expect.poll(() => music(page)).toEqual([expect.objectContaining({
    originalSrc: 'https://play.pokemonshowdown.com/audio/xy-trainer.mp3', paused: false, volume: 0.41,
  })]);

  await page.getByRole('link', { name: 'Settings', exact: true }).click();
  await page.getByLabel('Music track', { exact: true }).selectOption('bw-trainer');
  await returnToBattle(page);
  await expect.poll(() => music(page)).toEqual([
    expect.objectContaining({ originalSrc: 'https://play.pokemonshowdown.com/audio/xy-trainer.mp3', src: '', paused: true, loadCalls: 1 }),
    expect.objectContaining({ originalSrc: 'https://play.pokemonshowdown.com/audio/bw-trainer.mp3', paused: false, volume: 0.41 }),
  ]);
  await page.getByRole('link', { name: 'Settings', exact: true }).click();
