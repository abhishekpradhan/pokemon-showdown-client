...
const log = [
  '|gen|9', '|gametype|singles', '|player|p1|Alice|', '|player|p2|Bob|', '|tier|[Gen 9] OU',
  '|poke|p1|Pikachu|', '|poke|p2|Charizard|', '|start',
  '|switch|p1a: Pikachu|Pikachu|100/100', '|switch|p2a: Charizard|Charizard|100/100',
  '|turn|1', '|move|p1a: Pikachu|Thunderbolt|p2a: Charizard', '|-damage|p2a: Charizard|50/100', '|turn|2',
  '|move|p2a: Charizard|Flamethrower|p1a: Pikachu', '|-damage|p1a: Pikachu|50/100', '|turn|3', '|win|Alice',
].join('\n');
...
test('protected replay links retain their viewpoint and turn across reload', async ({ page }) => {
  await page.route('https://replay.pokemonshowdown.com/gen9ou-123-secretpw.json*', route => route.fulfill({ json: { log, private: 2, format: '[Gen 9] OU', players: ['Alice', 'Bob'] } }));
  ...
  await page.screenshot({ path: `/tmp/showdown-release-replay-${test.info().project.name}.png`, fullPage: true });
});

test('ladder player lookup survives reload without altering matchmaking selection', async ({ page }) => {
  await page.route('https://pokemonshowdown.com/ladder/*.json*', route => route.fulfill({ json: { toplist: [{ username: 'Alice', elo: 1650, gxe: 70.2, rpr: 1800, rprd: 90, w: 12, l: 4 }] } }));
  await page.route('https://pokemonshowdown.com/users/bob.json', route => route.fulfill({ json: { username: 'Bob', ratings: { gen9ou: { elo: 1150, gxe: 52.5, rpr: 1500, rprd: 125, w: 3, l: 2 } } } }));
  await page.goto('/');
  ...
  await page.screenshot({ path: `/tmp/showdown-release-ladder-${test.info().project.name}.png`, fullPage: true });
