...
const cases = captured.recordings.map(recording => {
  ...
  assert(
    !allLines.some(raw => /^\|(?:challstr|trn|pm)\|/.test(raw)),
    'Only battle-scoped corpus frames may be promoted.',
  );
...
writeFileSync(
  ...
  JSON.stringify(
    {
      source: `https://github.com/smogon/pokemon-showdown/tree/${captured.upstreamRevision}`,
      upstreamRevision: captured.upstreamRevision,
      provenance:
        'Recorded by scripts/test-local-server.mjs against loopback only. All names/teams are synthetic; auth/global frames and room HTML are excluded. Numeric battle IDs are normalized; remaining retained protocol lines are verbatim.',
      cases,
    },
    null,
    2,
