...
export const line = (command, predicate = () => true) => frame => frame.lines.some(item => item.command === command && predicate(item.args));
...
export function participant(ProtocolClient, config, name) {
  ...
  const entry = {
    name, client, frames, states, sent,
    send(command, room = '') { assert(client.send(command, room), `Unable to send ${command.split(' ')[0]}`); },
    close() { unsubscribe(); client.disconnect(); },
    wait(predicate, label, from = 0) {
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          listeners.delete(check);
          reject(new Error(`${name}: timeout waiting for ${label}. Last protocol: ${frames.slice(-3).map(frame => frame.raw).join('\n').slice(-2500)}`));
        }, 20_000);
        function check() {
          const frame = frames.slice(from).find(predicate);
          if (frame) { clearTimeout(timer); listeners.delete(check); resolve(frame); }
        }
...
export async function identify(player, from = 0) {
  ...
  await player.wait(line('updateuser', args => args[0].trim() === player.name && args[1] === '1'), 'guest identity', from);
}
...
export async function challenge(players, format) {
  ...
  await bob.wait(line('pm', args => args[0].trim() === alice.name && args[2]?.startsWith(`/challenge ${format}`)), `${format} challenge`, offsets[1]);
  bob.send(`/accept ${alice.name}`);
  const initial = await alice.wait(frame => frame.roomId.startsWith('battle-') && line('init', args => args[0] === 'battle')(frame), `${format} initialization`, offsets[0]);
...
export async function playUntil(players, room, { offsets = players.map(() => 0), turn = 2, choose = () => 'default', expectTeamSize = 6, expectedErrors = new Set() } = {}) {
  ...
  await Promise.all(players.map(async (player, playerIndex) => {
    ...
    while (true) {
      const frame = await player.wait(frame => frame.roomId === room && frame.lines.some(item => ['request', 'error', 'turn', 'win', 'tie'].includes(item.command)), 'next request or resolved turn', cursor);
      ...
      if (!kind) continue;
      assert(request.rqid > 0 && request.side.pokemon.length === expectTeamSize, 'Real simulator must provide the expected roster and request ID.');
      submitted.add(request.rqid); choices[kind]++;
...
export async function finishBattle(players, room, winner = players[1].name, offsets = players.map(() => 0)) {
  ...
  await Promise.all(players.map((player, index) => player.wait(frame => frame.roomId === room && line('win', args => args[0] === winner)(frame), 'authoritative winner', offsets[index])));
}
