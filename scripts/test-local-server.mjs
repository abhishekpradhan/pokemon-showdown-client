...
const runtimeHash = createHash('sha256').update(readFileSync(join(runtime, 'package-lock.json'))).digest('hex');
const installedHash = join(directory, '.arena-runtime-hash');
if (!existsSync(installedHash) || readFileSync(installedHash, 'utf8') !== runtimeHash || !existsSync(join(directory, 'node_modules/esbuild'))) {
  for (const name of ['package.json', 'package-lock.json']) copyFileSync(join(runtime, name), join(directory, name));
...
writeFileSync(join(directory, 'config/custom-formats.ts'), `export const Formats = [
...
function participant(name) {
  const entry = createParticipant(ProtocolClient, { id: 'arenalocal', host: '127.0.0.1', port, prefix: '/showdown', secure: false, loginServer: 'http://127.0.0.1:1/' }, name);
  clients.push(entry);
  return entry;
}
...
try {
  ...
  const replayReady = childMessage('replay-ready'); server.send({ type: 'prepare-replay' }); await replayReady;
  const replayOffset = alice.frames.length;
  alice.send('/hidereplay', initial.roomId);
  await alice.wait(frame => frame.roomId === initial.roomId && frame.raw.includes('hid the replay of this battle'), 'private replay permission acknowledged', replayOffset);
  const publication = childMessage('replay-published');
  alice.send('/savereplay', initial.roomId);
  const published = await publication;
  const replayPopup = await alice.wait(line('popup', args => args.join('|').includes('Your replay has been uploaded')), 'authoritative modern replay URL', replayOffset);
  ...
  await build({ entryPoints: [resolve('src/compat/replay-upload.ts')], bundle: true, platform: 'node', format: 'esm', outfile: replayParser,
  ...
  evidence.push('Modern private replay: real /hidereplay and /savereplay enforce participant permission and hidden=1; stub backend returns a password URL preserved by the actual client parser. No external publication.');
  ...
  evidence.push('Synthetic local OAuth token lifecycle and private legacy replay publishing use actual HTTP/adapters; cross-origin and redirected uploads fail closed.');
  cleanup();
  ...
  console.error(JSON.stringify(clients.map(player => ({ name: player.name, failures: player.frames.flatMap(frame => frame.lines.filter(item => ['error', 'popup'].includes(item.command)).map(item => item.args.join('|'))).slice(-3) })), null, 2));
  throw error;
} finally { cleanup(); }
