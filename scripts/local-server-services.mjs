...
export async function verifyLocalServices(directory) {
  ...
  const handleRequest = async (request, response) => {
    ...
    if (url.pathname === '/api/oauth/api/getassertion') {
      ...
      const accepted = url.searchParams.get('client_id') === 'arena-local' && url.searchParams.get('token') === token && !revoked;
      response.end(`]${JSON.stringify(accepted ? { success: true, user: 'ArenaRegistered', data: `${url.searchParams.get('challenge')};local-signature` } : { success: false })}`);
      ...
      if (replayMode === 'redirect') { response.writeHead(302, { Location: 'https://example.invalid/must-not-follow' }); response.end('not an upload'); }
      else response.end('success:arenalocal-gen9ou-1-local-private-tokenpw');
    } else { response.statusCode = 404; response.end('Unknown local test endpoint'); }
  };
  ...
  Object.defineProperty(navigator, 'locks', { configurable: true, value: { request: (_name, options, callback) => {
    const job = pendingLock.then(() => { if (options.signal.aborted) throw options.signal.reason; return callback(); });
  ...
  try {
    ...
    await build({ stdin: { contents: `export * from './src/compat/ps-oauth.ts'; export {default as replay} from './api/replay.ts';`, resolveDir: resolve('.') }, bundle: true, platform: 'node', format: 'esm', outfile: file,
      define: { 'import.meta.env': JSON.stringify({ VITE_PS_OAUTH_ROOT: `${origin}/api`, VITE_PS_OAUTH_CLIENT_ID: 'arena-local' }) } });
    ...
    const current = await Promise.all([service.currentOAuthToken(new AbortController().signal), service.currentOAuthToken(new AbortController().signal)]);
    assert.equal(events.filter(event => event === 'refresh').length, 1, 'Concurrent sessions must rotate once under the shared lock.');
    assert(current.every(record => record.token === token && record.user === 'ArenaRegistered'));
    assert.equal(await service.assertionFromToken('1|old', priorToken), null, 'Provider must reject a rotated token.');
    ...
    const body = new URLSearchParams({ id: 'arenalocal-gen9ou-1', serverid: 'arenalocal', log: '|gen|9\n|turn|1', password: 'local-private-token' });
    const request = (extra = {}) => new Request(`${origin}/api/replay`, { method: 'POST', headers: { Origin: origin, 'Content-Type': 'application/x-www-form-urlencoded', Cookie: 'unrelated-user-cookie', Authorization: 'Bearer unrelated', ...extra }, body });
    ...
    return { provider: 'synthetic loopback HTTP, published OAuth token and legacy replay contracts', tokenRotation: true, concurrentRefreshes: 1, rotatedTokenRejected: true, revocation: true, expiry: true, privateLegacyReplay: true, crossOriginBlocked: true, redirectBlocked: true, events };
  } finally {
    if (originalLogin === undefined) delete process.env.PS_LOGIN_SERVER; else process.env.PS_LOGIN_SERVER = originalLogin;
