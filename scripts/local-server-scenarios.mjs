...
const team = Teams =>
  Teams.pack(
    Array.from({ length: 6 }, (_, index) => ({
      ...
      species: 'Mew',
      ability: 'Synchronize',
      moves: ['Tackle', 'Helping Hand', 'Splash'],
      nature: 'Hardy',
      level: 100,
    })),
  );
...
export async function verifyProfileConfirmation({ player, evidence, setThrottle }) {
  ...
  const ownDetails = avatar =>
    line(
      'queryresponse',
      args =>
        args[0] === 'userdetails' &&
        JSON.parse(args[1]).userid === userid &&
        JSON.parse(args[1]).avatar === avatar,
    );
  ...
  await player.wait(
    ownDetails('lucas'),
    'seeded avatar confirmed before enabling normal throttling',
    seededFrom,
  );
  ...
  try {
    ...
    const primer = await player.wait(
      line('updateuser', args => args[2] === 'lucas' && JSON.parse(args[3] || '{}').language === 'english'),
      'normal command queue primed',
      from,
    );
    const stale = await player.wait(ownDetails('lucas'), 'exempt cmd query demonstrates stale avatar', from);
    const preview = await player.wait(
      frame =>
        frame.lines.some(
          item =>
            (item.command === 'raw' || (item.command === 'pm' && item.args[2]?.startsWith('/raw '))) &&
            item.args.join('|').includes('/dawn.png'),
        ),
      'queued avatar preview',
      from,
    );
    const confirmed = await player.wait(ownDetails('dawn'), 'queued query confirms updated own avatar', from);
    const details = JSON.parse(
      confirmed.lines.find(item => item.command === 'queryresponse' && item.args[0] === 'userdetails')
        .args[1],
    );
    assert.equal(details.name, player.name);
    assert(
      player.frames.indexOf(stale) < player.frames.indexOf(preview),
      'The exempt query must overtake the queued avatar change.',
    );
    assert(
      player.frames.indexOf(preview) < player.frames.indexOf(confirmed),
      'The queued query must confirm after the avatar mutation.',
    );
    const avatarFrames = player.frames.slice(
      player.frames.indexOf(primer) + 1,
      player.frames.indexOf(confirmed) + 1,
    );
    assert(
      !avatarFrames.some(line('updateuser')),
      '/avatar must not be mistaken for the language updateuser contract.',
    );
    const languageFrom = player.frames.length;
    player.send('/language french');
    const languageFrame = await player.wait(
      line('updateuser', args => JSON.parse(args[3] || '{}').language === 'french'),
      'server language metadata confirmation',
      languageFrom,
    );
    ...
    player.send('/language english');
    await player.wait(
      line('updateuser', args => JSON.parse(args[3] || '{}').language === 'english'),
      'restore English for later scenario assertions',
      resetFrom,
    );
    evidence.push(
      'Real throttled profile confirmation: exempt /cmd userdetails overtakes /avatar and returns stale Lucas; queued /query userdetails returns Dawn after its raw preview. /language french confirms authoritative updateuser metadata.',
    );
    return {
      normalGuestThrottling: true,
      queueImplementation: 'unmodified upstream guest command queue',
      initialAvatar: 'lucas',
      exemptCmdReturnedStaleAvatar: true,
      avatar: 'dawn',
      avatarPreview: preview.lines.some(item => item.command === 'pm') ? 'PM /raw directive' : 'raw command',
      avatarConfirmation: 'queued /query userdetails',
      avatarUpdateuserBeforeQuery: false,
      language: 'french',
      languageConfirmation: 'updateuser settings.language',
      englishRestored: true,
    };
  } finally {
...
export async function verifyReconnect({ alice, bob, Teams, evidence }) {
  ...
  const request = await alice.wait(
    frame => frame.roomId === room && line('request')(frame),
  ...
  const snapshot = await alice.wait(
    frame => frame.roomId === room && line('init', args => args[0] === 'battle')(frame),
    'reconnect battle snapshot',
    from,
  );
  assert(snapshot.lines.some(item => item.command === 'player' && item.args[1] === alice.name));
  await playUntil([alice, bob], room, {
    offsets: [from, offsets[1]],
    choose: ({ kind }) => (kind === 'move' ? 'move 1 1, move 1 2' : 'default'),
  });
  await finishBattle([alice, bob], room, bob.name, [from, offsets[1]]);
  assert(
    !alice.sent.some(message => message.endsWith('|999')),
    'A disconnected choice must never reach the replacement transport.',
  );
  evidence.push(
    'Real active doubles reconnect: fresh identity, complete room/request snapshot, stale private choice rejected, explicit target choices and authoritative result.',
  );
}
...
export async function verifyLayouts({ alice, bob, participant, Teams, evidence, revision }) {
  ...
  const saveCorpus = () =>
    writeFileSync(
      resolve('test-results-local-corpus.json'),
      JSON.stringify(
        {
          upstreamRevision: revision,
          note: 'Synthetic local players and teams only; no authentication or live-user data.',
          recordings,
        },
        null,
        2,
  ...
  for (const [format, gameType, count] of [
      ...
      for (const [index, player] of players.entries()) {
        ...
        await player.wait(
          line(
            'pm',
            args =>
              args[2] === `/challenge ${format}` &&
              args.join('|').includes("You're invited to join a battle"),
          ),
          'four-seat invitation',
          offsets[index],
        );
    ...
    const choices = await playUntil(players, room, {
      ...
      choose: async ({ player, playerIndex, request, kind }) => {
        ...
        if (gameType === 'triples' && playerIndex === 0 && moveIndex === 0) {
          ...
          const rejected = await player.wait(
            frame => frame.roomId === room && line('error', args => /target/i.test(args.join('|')))(frame),
            'far triples target rejected',
            from,
          );
        ...
        if (gameType === 'triples')
          return playerIndex === 0 && moveIndex === 1
            ? 'shift, move 1 2, move 1 1'
            : 'move 1 3, move 1 2, move 1 1';
    ...
    if (gameType === 'triples') {
      ...
      assert(
        allFrames.some(frame => line('swap')(frame)),
        'Triples edge shift must resolve on the real simulator.',
      );
    }
    if (gameType === 'multi')
      assert(
        allFrames.some(frame =>
          line(
            'move',
            args => args[0].startsWith('p3') && args[1] === 'Helping Hand' && args[2].startsWith('p1'),
          )(frame),
        ),
        'p3 signed ally target must resolve to p1.',
      );
    if (gameType === 'freeforall')
      assert(
        allFrames.some(frame =>
          line('move', args => args[0].startsWith('p3') && args[2].startsWith('p1'))(frame),
        ),
        'FFA negative target must resolve to the same-parity opponent.',
      );
    for (const [index, player] of players.entries())
      recordings.push({
        scenario: gameType,
        seat: `p${index + 1}`,
        frames: player.frames.filter(frame => frame.roomId === room).map(frame => frame.raw),
      });
    saveCorpus();
    if (count === 2) await finishBattle(players, room, bob.name, offsets);
      ...
      await bob.wait(
        frame => frame.roomId === room && line('win')(frame),
        'four-seat authoritative result',
        from,
      );
    }
    layouts.push({
      ...
      players: count,
      choices,
      rejectedInvalidTargets: expectedErrors.size,
      ...(count === 4
        ? { invitationAcceptance: { p3: '/accept', p4: '/acceptbattle', bothReceivedOwnRequest: true } }
        : {}),
    });
    evidence.push(
      `${gameType}: ${count} real seats, exact signed targets, full roster/request IDs, resolved actions${gameType === 'triples' ? ', rejected far target and edge shift' : ''}.`,
    );
