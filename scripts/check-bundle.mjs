...
const assets = new Set(
  JSON.parse(offlineManifest).assets.map(path => (path === '/' ? 'index.html' : path.slice(1))),
);
let allRaw = 0;
let allGzip = 0;
for (const file of assets) {
  const body = readFileSync(resolve('dist', file));
  allRaw += statSync(resolve('dist', file)).size;
  allGzip += gzipSync(body).length;
}
...
for (const key of Object.keys(budgets)) {
  if (actual[key] > budgets[key])
    throw new Error(`${key} exceeds its reviewed bundle budget (${actual[key]} > ${budgets[key]})`);
}
