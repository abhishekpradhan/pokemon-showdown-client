...
export function resolveLoginTarget(env) {
  if (env.LIVE_APP_URL !== undefined) {
    let app;
    try { app = new URL(env.LIVE_APP_URL); }
    catch { throw new SmokeFailure('LIVE_APP_URL must be an absolute application origin.'); }
    ...
    if (app.username || app.password || app.pathname !== '/' || app.search || app.hash ||
      ...
      throw new SmokeFailure('LIVE_APP_URL must be an HTTPS origin without credentials, path, query or fragment (HTTP is allowed for loopback).');
    }
    return { endpoint: `${app.origin}/api/action`, origin: app.origin, mode: 'deployed-proxy' };
  }
  let endpoint;
  try { endpoint = new URL(env.PS_LOGIN_SERVER || 'https://play.pokemonshowdown.com/action.php'); }
  catch { throw new SmokeFailure('PS_LOGIN_SERVER must be an absolute HTTP(S) URL.'); }
...
export async function runLiveSmoke({ env = process.env, WebSocketImpl = globalThis.WebSocket, fetchImpl = globalThis.fetch, output = console, timeoutMs = 25_000 } = {}) {
  if (env.LIVE_PS_TESTS !== '1') {
    output.log('Skipping live PS smoke test. Set LIVE_PS_TESTS=1 to run it.');
    return { skipped: true };
  }
  if (!WebSocketImpl) throw new SmokeFailure('This runtime does not provide WebSocket; use supported Node 22 or 24.');
  ...
  return new Promise((resolveResult, reject) => {
    ...
    const timeout = setTimeout(() => finish('Timed out before the signed guest handshake completed.'), timeoutMs);
    try { socket = new WebSocketImpl(url); }
    catch { finish('Unable to open the configured simulator socket.'); return; }
    ...
    socket.addEventListener('close', () => { if (!settled) finish('Simulator disconnected before the guest handshake completed.'); });
      ...
      for (const line of frame.split('\n')) {
        ...
        if (line.startsWith('|challstr|') && !requestingAssertion) {
          ...
          const response = await fetchImpl(target.endpoint, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded', ...(target.origin ? { Origin: target.origin } : {}) },
            body: new URLSearchParams({ act: 'getassertion', userid, challstr: line.slice('|challstr|'.length) }),
            signal: abort.signal,
            redirect: 'error',
          });
          ...
          if (!response.ok) {
            ...
            finish(`The ${target.mode === 'deployed-proxy' ? 'deployed assertion proxy' : 'login server'} returned HTTP ${response.status}.`);
            return;
          }
        ...
        if (line.startsWith('|nametaken|')) { finish('The simulator rejected the ephemeral guest name.'); return; }
    ...
    socket.addEventListener('message', event => {
      void handleFrame(String(event.data)).catch(() => finish('The assertion request or simulator handshake failed.'));
    });
  });
}
